...
import {
  generateAphantasiaCard,
  AphantasiaScoreResult,
  AphantasiaCardLang,
} from "@/lib/generate-quiz-card";

interface AphantasiaShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: AphantasiaScoreResult;
  lang: AphantasiaCardLang;
}

export const AphantasiaShareCardModal: React.FC<AphantasiaShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateAphantasiaCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Aphantasia card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { dataUrl } = await generateAphantasiaCard(result, lang);
      const link = document.createElement("a");
      link.download = `nuju-aphantasia-${result.level}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleShare = async () => {
    try {
      const { file } = await generateAphantasiaCard(result, lang);
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "My Mind's Eye & Visual Imagery Profile (#100 Milestone)",
          text: `I took the Aphantasia & Mind's Eye Blindness Screener (VVIQ Model) on Nuju. My visualization vividness index is ${result.percentage}%. Check yours at:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/aphantasia");
      setCopied(true);
      ...
      <div className="relative w-full max-w-md bg-neutral-900 border border-sky-800/40 rounded-3xl p-6 shadow-2xl overflow-hidden flex flex-col items-center">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-sky-300">
            Share Story Card · #100 Milestone (1080x1350)
          </span>
        </div>
        ...
        <div className="w-full max-h-[440px] aspect-[4/5] bg-black/50 rounded-2xl border border-white/10 overflow-hidden flex items-center justify-center relative mb-6">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Aphantasia Story Card Preview"
        ...
        <div className="w-full grid grid-cols-2 gap-3">
          <button
            ...
            disabled={generating}
            className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-semibold text-xs transition shadow-md disabled:opacity-50"
          >
        ...
        >
          {copied ? (
            ...
            <>
              ...
              <span>Copy test link: nuju.app/quiz/aphantasia</span>
            </>
          )}
        </button>
      </div>
