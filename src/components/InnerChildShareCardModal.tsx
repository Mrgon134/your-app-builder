...
import { generateInnerChildCard } from "@/lib/generate-quiz-card";

interface InnerChildShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const InnerChildShareCardModal: React.FC<InnerChildShareCardModalProps> = ({
  isOpen,
  onClose,
  profile,
  woundType,
  ...
  useEffect(() => {
    ...
    generateInnerChildCard(profile, woundType, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Inner Child card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  }, [isOpen, profile, woundType, lang]);

  const handleDownload = () => {
    if (!cardData) return;
    const a = document.createElement("a");
    a.href = cardData.dataUrl;
    a.download = `ju-inner-child-${woundType}.png`;
  ...
  const handleShare = async () => {
    if (!cardData) return;
    const shareText = `My Inner Child Wound: ${(profile.title as any)[lang] || profile.title.en} - ${(profile.archetype as any)[lang] || profile.archetype.en}\nDiscover yours free at: ${window.location.origin}/quiz/inner-child`;

    if (navigator.share && navigator.canShare && navigator.canShare({ files: [cardData.file] })) {
      try {
        await navigator.share({
          title: "My Inner Child Archetype Result",
          text: shareText,
          files: [cardData.file],
        });
      ...
      <DialogContent className="sm:max-w-md max-h-[90vh] flex flex-col p-6 bg-slate-900 border-slate-800 text-slate-100 rounded-3xl overflow-hidden">
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          <button
            onClick={handleShare}
            disabled={!cardData || isGenerating}
            className="flex-1 py-3 px-4 rounded-xl bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-pink-500/20 transition disabled:opacity-50"
          >
...
export default InnerChildShareCardModal;
