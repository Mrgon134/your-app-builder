...
import {
  PerceivedLang,
  PerceivedScoreResult,
} from "@/data/fear-of-being-perceived";
import { generateFearOfBeingPerceivedCard } from "@/lib/generate-quiz-card";

interface FearOfBeingPerceivedShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const FearOfBeingPerceivedShareCardModal: React.FC<
  FearOfBeingPerceivedShareCardModalProps
> = ({ isOpen, onClose, result, lang }) => {
  ...
  useEffect(() => {
    ...
    generateFearOfBeingPerceivedCard(result, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Fear of Being Perceived card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `nuju-fear-perceived-${result.level}.png`;
    ...
    if (
      ...
      try {
        await navigator.share({
          title: "My Fear of Being Perceived Profile",
          text: `My Visibility Archetype: ${
            result.profile.title[lang] || result.profile.title.en
          } (${result.percentage}% perceived load index). Screen your spotlight anxiety free:`,
          url: "https://nuju.app/quiz/fear-of-being-perceived",
          files: [cardData.file],
        });
      } catch (err) {
  ...
  const fallbackCopy = () => {
    navigator.clipboard.writeText("https://nuju.app/quiz/fear-of-being-perceived");
    setIsCopied(true);
      ...
      <DialogContent className="sm:max-w-md max-w-[92vw] bg-neutral-900 border-neutral-800 text-white p-6 rounded-3xl overflow-hidden shadow-2xl">
        <DialogHeader className="text-left space-y-2">
          ...
          <DialogTitle className="text-xl font-bold text-white tracking-tight">
            Export Your Spotlight Card
          </DialogTitle>
          <DialogDescription className="text-xs text-neutral-400">
            Share your visibility dread metrics, covert concealment score, and safe exposure roadmap.
          </DialogDescription>
        </DialogHeader>
          ...
          {isGenerating ? (
            ...
            <div className="relative group w-full flex justify-center">
              <img
                ...
                alt="Fear of Being Perceived Story Card"
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={handleShare}
            disabled={!cardData}
            className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:opacity-90 active:scale-[0.98] text-white font-semibold text-sm shadow-lg shadow-emerald-500/20 transition-all disabled:opacity-50 disabled:pointer-events-none"
          >
