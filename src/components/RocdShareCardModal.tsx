...
interface RocdShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const RocdShareCardModal: React.FC<RocdShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateRocdCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate ROCD card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { file } = await generateRocdCard(result, lang);
      const url = URL.createObjectURL(file);
      const a = document.createElement("a");
      a.href = url;
      a.download = `nuju-rocd-${result.level}.png`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: "My Relationship OCD (ROCD) Assessment Result | Nuju",
          text: `My ROCD Obsessive Intensity Index is ${result.percentage}%. Check your relationship checking and doubt patterns on Nuju:`,
          url: "https://nuju.app/quiz/rocd",
        });
      } else {
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/rocd");
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-md rounded-3xl border border-purple-500/30 bg-gradient-to-b from-[#210D34] to-[#0A0311] p-6 text-white shadow-2xl">
        <button
          onClick={onClose}
        ...
        <div className="text-center mb-4">
          ...
          <h3 className="text-lg font-bold">
            {lang === "id" ? "Bagikan Hasil ROCD Anda" : "Share Your ROCD Metric"}
          </h3>
        ...
        <div className="relative mx-auto my-4 aspect-[4/5] max-h-[380px] w-auto overflow-hidden rounded-2xl border border-purple-500/30 shadow-inner bg-black/40 flex items-center justify-center">
          {generating || !cardPreview ? (
            ...
            <img
              src={cardPreview}
              alt="Relationship OCD Result Card"
        ...
        <div className="mt-6 flex flex-col gap-2.5">
          <button
            ...
            disabled={generating}
            className="w-full flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 py-3 text-xs sm:text-sm font-bold text-white shadow-lg hover:opacity-90 transition disabled:opacity-50"
          >
