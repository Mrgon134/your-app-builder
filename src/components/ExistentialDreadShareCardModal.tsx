...
import { generateExistentialDreadCard } from "@/lib/generate-quiz-card";

interface ExistentialDreadShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: ExistentialDreadScoreResult;
  lang: ExistentialDreadLang;
}

export const ExistentialDreadShareCardModal: React.FC<
  ExistentialDreadShareCardModalProps
> = ({ isOpen, onClose, result, lang }) => {
  ...
  useEffect(() => {
    ...
    generateExistentialDreadCard(result, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Existential Dread card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `nuju-existential-${result.level}.png`;
    ...
    if (
      ...
      try {
        await navigator.share({
          title: "My Existential Purpose Profile",
          text: `My Existential Archetype: ${
            result.profile.title[lang] || result.profile.title.en
          } (${result.percentage}% meaning alignment). Screen your purpose score:`,
          url: "https://nuju.app/quiz/existential-dread",
          files: [cardData.file],
        });
      } catch (err) {
  ...
  const fallbackCopy = () => {
    navigator.clipboard.writeText("https://nuju.app/quiz/existential-dread");
    setIsCopied(true);
      ...
      <DialogContent className="sm:max-w-md max-w-[92vw] bg-neutral-900 border-neutral-800 text-white p-6 rounded-3xl overflow-hidden shadow-2xl">
        <DialogHeader className="text-left space-y-2">
          ...
          <DialogTitle className="text-xl font-bold text-white tracking-tight">
            Export Your Meaning Card
          </DialogTitle>
          <DialogDescription className="text-xs text-neutral-400">
            Share your purpose alignment, temporal anxiety score, and existential sovereignty roadmap.
          </DialogDescription>
        </DialogHeader>
          ...
          {isGenerating ? (
            ...
            <div className="relative group w-full flex justify-center">
              <img
                ...
                alt="Existential Dread Story Card"
