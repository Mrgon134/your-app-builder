...
import {
  generateComparisonTrapCard,
  ComparisonTrapScoreResult,
  ComparisonTrapCardLang,
} from "@/lib/generate-quiz-card";

interface ComparisonTrapShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: ComparisonTrapScoreResult;
  lang: ComparisonTrapCardLang;
}

export const ComparisonTrapShareCardModal: React.FC<ComparisonTrapShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateComparisonTrapCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Comparison Trap card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { file } = await generateComparisonTrapCard(result, lang);
      const url = URL.createObjectURL(file);
      const a = document.createElement("a");
      a.href = url;
      a.download = `nuju-comparison-trap-${result.level}.png`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: "My Comparison Trap & Social Media Envy Result | Nuju",
          text: `My Social Comparison & Envy Index is ${result.percentage}%. Screen upward comparison and social media burnout on Nuju:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/comparison-trap");
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-3xl p-6 text-white shadow-2xl my-8">
        ...
        <h3 className="text-xl font-bold mb-1">Your Social Comparison Card</h3>
        <p className="text-xs text-neutral-400 mb-5">
          Download or share your envy & comparison profile for Instagram Stories, TikTok, or private grounded reflection.
        </p>
        ...
        <div className="relative w-full aspect-[4/5] max-h-[460px] bg-neutral-950 rounded-2xl overflow-hidden border border-neutral-800 mb-6 flex items-center justify-center">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Comparison Trap Card Preview"
        ...
        <div className="flex flex-col gap-2.5">
          <div className="grid grid-cols-2 gap-2.5">
            <button
              onClick={handleShare}
              disabled={generating}
              className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-cyan-500 hover:bg-cyan-400 font-semibold text-sm transition disabled:opacity-50 text-neutral-950"
            >
          ...
          >
            ...
            <span>{copied ? "Link copied to clipboard!" : "Copy test link to escape the comparison trap"}</span>
          </button>
        </div>
      </div>
    </div>
