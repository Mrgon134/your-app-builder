...
import { Download, Share2, Copy, Check, Sparkles, Loader2, Heart } from "lucide-react";
import { toast } from "sonner";
import { LoveLanguageResult, LoveLang } from "@/data/love-languages";
import { generateLoveLanguageCard } from "@/lib/generate-quiz-card";

interface LoveLanguageShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const LoveLanguageShareCardModal: React.FC<LoveLanguageShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  useEffect(() => {
    ...
    generateLoveLanguageCard(result, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Love Language card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `ju-love-language-${result.primary.id}.png`;
  ...
  const handleShare = async () => {
    ...
    const shareText = `My Primary Love Language: ${titleText} (${result.percentages[result.primary.id]}%)\nDiscover yours free at: ${window.location.origin}/quiz/love-languages`;

    if (navigator.share && navigator.canShare && navigator.canShare({ files: [cardData.file] })) {
      try {
        await navigator.share({
          title: "My Love Language Assessment Result",
          text: shareText,
          files: [cardData.file],
        });
      ...
      <DialogContent className="sm:max-w-md max-h-[90vh] flex flex-col p-6 bg-slate-900 border-slate-800 text-slate-100 rounded-3xl overflow-hidden">
        <DialogHeader className="text-left space-y-1">
          <DialogTitle className="text-xl font-bold flex items-center gap-2 text-white">
            ...
            <span>{lang === "id" ? "Bagikan Kartu Bahasa Cinta" : "Share Love Language Card"}</span>
          </DialogTitle>
          <DialogDescription className="text-xs text-slate-400">
            {lang === "id"
              ? "Kartu estetis format Instagram Story (9:16) siap unduh atau bagikan ke pasangan."
              : "High-resolution 9:16 Instagram Story card ready to download or share with your partner."}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          <button
            onClick={handleShare}
            disabled={!cardData || isGenerating}
            className="flex-1 py-3 px-4 rounded-xl bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-rose-500/20 transition disabled:opacity-50"
          >
...
export default LoveLanguageShareCardModal;
