...
import {
  generateAbandonmentSchemaCard,
  AbandonmentSchemaScoreResult,
  AbandonmentSchemaCardLang,
} from "@/lib/generate-quiz-card";

interface AbandonmentSchemaShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: AbandonmentSchemaScoreResult;
  lang: AbandonmentSchemaCardLang;
}

export const AbandonmentSchemaShareCardModal: React.FC<AbandonmentSchemaShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateAbandonmentSchemaCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Abandonment Schema card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { file } = await generateAbandonmentSchemaCard(result, lang);
      const url = URL.createObjectURL(file);
      const a = document.createElement("a");
      a.href = url;
      a.download = `nuju-abandonment-schema-${result.level}.png`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: "My Abandonment Schema & Relational Panic Result | Nuju",
          text: `My Abandonment Schema Load is ${result.percentage}%. Screen relational panic & schema therapy patterns on Nuju:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/abandonment-schema");
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-3xl p-6 text-white shadow-2xl my-8">
        ...
        <h3 className="text-xl font-bold mb-1">Your Abandonment Pattern Card</h3>
        <p className="text-xs text-neutral-400 mb-5">
          Download or share your Jeffrey Young schema therapy attachment profile for self-soothing or relationship awareness.
        </p>
        ...
        <div className="relative w-full aspect-[4/5] max-h-[460px] bg-neutral-950 rounded-2xl overflow-hidden border border-neutral-800 mb-6 flex items-center justify-center">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Abandonment Schema Card Preview"
        ...
        <div className="flex flex-col gap-2.5">
          ...
          >
            {copied ? <Check className="w-3.5 h-3.5 text-rose-300" /> : <Copy className="w-3.5 h-3.5" />}
            <span>{copied ? "Link copied to clipboard!" : "Copy test link to screen abandonment schema"}</span>
          </button>
        </div>
      </div>
    </div>
