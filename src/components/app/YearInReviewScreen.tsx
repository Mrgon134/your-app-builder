...
import { ArrowLeft, BookOpen, Flame, Sparkles, Star, TrendingUp, Trophy, Zap } from "lucide-react";
      ...
      {!stats ? (
        <div className="text-center py-16">
          ...
          <p className="text-[15px] text-muted-foreground">
            {(t.year_empty || "Start with one honest check-in to see your {year} in review").replace("{year}", String(year))}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          >
            ...
            <p className="text-[15px] text-muted-foreground mt-1">
              {(t.year_entries || "journal entries in {year}").replace("{year}", String(year))}
            </p>
            <div className="flex items-center justify-center gap-1.5 mt-2">
              <Sparkles className="w-3.5 h-3.5 text-primary/60" />
              <p className="text-[13px] text-primary/70 font-medium">
                {Math.round((stats.total / 365) * 100)}
                {t.year_days_pct || "% of days journaled"}
              </p>
            </div>
          </div>
          ...
          <div className="glass-card rounded-2xl p-5">
            <p className="text-[11px] font-semibold text-primary uppercase tracking-widest mb-4">
              {t.year_mood_dist || "Mood Distribution"}
            </p>
          ...
          <div className="glass-card rounded-2xl p-5">
            <p className="text-[11px] font-semibold text-primary uppercase tracking-widest mb-4">
              {t.year_monthly_activity || "Monthly Activity"}
            </p>
