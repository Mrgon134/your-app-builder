...
import { generateChronicGuiltCard } from "@/lib/generate-quiz-card";

interface ChronicGuiltShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const ChronicGuiltShareCardModal: React.FC<
  ChronicGuiltShareCardModalProps
> = ({ isOpen, onClose, result, lang }) => {
  ...
  useEffect(() => {
    ...
    generateChronicGuiltCard(result, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Chronic Guilt card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `nuju-chronic-guilt-${result.level}.png`;
    ...
    if (
      ...
      try {
        await navigator.share({
          title: "My Chronic Guilt & Self-Sabotage Profile",
          text: `My Toxic Burden Archetype: ${
            result.profile.title[lang] || result.profile.title.en
          } (${result.percentage}% guilt index). Unmask your subconscious remorse free:`,
          url: "https://nuju.app/quiz/chronic-guilt",
          files: [cardData.file],
        });
      } catch (err) {
  ...
  const fallbackCopy = () => {
    navigator.clipboard.writeText("https://nuju.app/quiz/chronic-guilt");
    setIsCopied(true);
      ...
      <DialogContent className="sm:max-w-md max-w-[92vw] bg-neutral-900 border-neutral-800 text-white p-6 rounded-3xl overflow-hidden shadow-2xl">
        <DialogHeader className="text-left space-y-2">
          ...
          <DialogTitle className="text-xl font-bold text-white tracking-tight">
            Export Your Toxic Guilt Card
          </DialogTitle>
          <DialogDescription className="text-xs text-neutral-400">
            Share your omnipotent responsibility score, punitive self-sabotage levels, and self-compassion action plan.
          </DialogDescription>
        </DialogHeader>
          ...
          {isGenerating ? (
            ...
            <div className="relative group w-full flex justify-center">
              <img
                ...
                alt="Chronic Guilt & Self-Sabotage Story Card"
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={handleShare}
            disabled={!cardData}
            className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-r from-rose-500 to-amber-600 hover:opacity-90 active:scale-[0.98] text-white font-semibold text-sm shadow-lg shadow-rose-500/20 transition-all disabled:opacity-50 disabled:pointer-events-none"
          >
