...
import { Download, Share2, Check, Loader2, HeartHandshake } from "lucide-react";
import { toast } from "sonner";
import { RsdLang } from "@/data/rsd-screener";
import { generateRsdCard, RsdCardInput } from "@/lib/generate-quiz-card";
...
export const RsdShareCardModal: React.FC<RsdShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  const handleDownload = () => {
    ...
    a.download = `ju-rsd-${result.level}.png`;
  ...
  const handleNativeShare = async () => {
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [cardData.file] })) {
      try {
        await navigator.share({
          title: "My Rejection Sensitive Dysphoria (RSD) Profile",
          text: `My RSD vulnerability score: ${result.profile.title[lang]} (${result.profile.badge[lang]}). Screen your emotional sensitivity on Ju:`,
          url: "https://www.nuju.app/quiz/rsd",
          files: [cardData.file],
        });
    ...
    if (navigator.clipboard) {
      try {
        await navigator.clipboard.writeText("https://www.nuju.app/quiz/rsd");
        setIsCopied(true);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md bg-card/95 backdrop-blur-xl border border-rose-500/30 p-6 rounded-3xl shadow-2xl">
        <DialogHeader className="space-y-1">
          ...
          <DialogTitle className="text-xl font-bold tracking-tight text-foreground">
            {lang === "id" ? "Bagikan Hasil Profil RSD Anda" : "Share Your RSD Profile"}
          </DialogTitle>
        ...
        <div className="my-3 flex flex-col items-center justify-center">
          {isGenerating ? (
            ...
            <div className="w-full relative group rounded-2xl overflow-hidden shadow-xl border border-rose-500/20">
              <img
                src={cardData.dataUrl}
                alt="RSD Story Share Card"
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            ...
            className="w-full py-3 px-4 rounded-xl bg-rose-500 hover:bg-rose-600 text-white font-bold text-xs flex items-center justify-center gap-2 transition-all shadow-md shadow-rose-500/20 disabled:opacity-50"
          >
