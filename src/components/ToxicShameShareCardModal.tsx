...
import { generateToxicShameCard } from "@/lib/generate-quiz-card";

interface ToxicShameShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const ToxicShameShareCardModal: React.FC<
  ToxicShameShareCardModalProps
> = ({ isOpen, onClose, result, lang }) => {
  ...
  useEffect(() => {
    ...
    generateToxicShameCard(result, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Toxic Shame card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    link.download = `nuju-toxic-shame-${result.level}.png`;
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [cardData.file] })) {
      try {
        await navigator.share({
          title: "My Toxic Shame & Inner Critic Profile",
          text: `My Toxic Shame Burden: ${result.percentage}% (${result.profile.title.en}). Screen your inner critic free on Nuju:`,
          files: [cardData.file],
        });
  ...
  const copyShareText = () => {
    const text = `My Toxic Shame Score: ${
      result.profile.title[lang] || result.profile.title.en
    } (${result.percentage}% defectiveness index - ${
      result.profile.badge[lang] || result.profile.badge.en
    }). Screen your inner critic free at: https://nuju.app/quiz/toxic-shame`;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md bg-[#1F1526] text-neutral-100 border-neutral-800 p-6 sm:p-8 rounded-3xl max-h-[92vh] overflow-y-auto">
        <DialogHeader className="text-left space-y-2">
          ...
          <DialogTitle className="text-xl font-bold text-white tracking-tight">
            Share Your Toxic Shame Profile
          </DialogTitle>
          <DialogDescription className="text-xs text-neutral-400">
            Export a high-resolution story card with your defectiveness index, self-contempt score, and somatic shame de-escalation protocol.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2.5">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-xl font-semibold text-sm bg-gradient-to-r from-amber-500 to-rose-500 hover:from-amber-400 hover:to-rose-400 text-neutral-950 font-bold transition shadow-lg shadow-amber-500/25 disabled:opacity-50 disabled:cursor-not-allowed"
          >
