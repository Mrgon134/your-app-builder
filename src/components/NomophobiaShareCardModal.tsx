...
import {
  generateNomophobiaCard,
  NomophobiaScoreResult,
  NomophobiaCardLang,
} from "@/lib/generate-quiz-card";

interface NomophobiaShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: NomophobiaScoreResult;
  lang: NomophobiaCardLang;
}

export const NomophobiaShareCardModal: React.FC<NomophobiaShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateNomophobiaCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Nomophobia card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { dataUrl } = await generateNomophobiaCard(result, lang);
      const link = document.createElement("a");
      link.download = `nuju-nomophobia-${result.level}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleShare = async () => {
    try {
      const { file } = await generateNomophobiaCard(result, lang);
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "My Nomophobia & Smartphone Separation Profile (Yildirim NMP-Q)",
          text: `I took the Nomophobia (Smartphone Separation Anxiety) Screener on Nuju. My digital dependency index is ${result.percentage}%. Screen yours free at:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/nomophobia");
      setCopied(true);
      ...
      <div className="relative w-full max-w-md bg-neutral-900 border border-cyan-800/40 rounded-3xl p-6 shadow-2xl overflow-hidden flex flex-col items-center">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-cyan-300">
            Share Story Card · Card #111 (1080x1350)
          </span>
        </div>

        <h3 className="text-xl font-bold text-white text-center mb-1">
          {lang === "id" ? "Kartu Hasil Nomofobia" : "Your Nomophobia Story Card"}
        </h3>
        ...
        <div className="flex items-center gap-3 w-full">
          <button
            ...
            disabled={generating}
            className="flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-cyan-500 hover:bg-cyan-600 active:scale-95 text-neutral-950 font-bold text-sm shadow-md transition disabled:opacity-50"
          >
