...
import { generateAdhdCard } from "@/lib/generate-quiz-card";

interface AdhdShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const AdhdShareCardModal: React.FC<AdhdShareCardModalProps> = ({
  isOpen,
  onClose,
  profile,
  totalScore,
  ...
  useEffect(() => {
    ...
    generateAdhdCard(profile, totalScore, inattentionScore, hyperactivityScore, dopamineScore, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating ADHD card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  }, [isOpen, profile, totalScore, inattentionScore, hyperactivityScore, dopamineScore, lang]);

  const handleDownload = () => {
    if (!cardData) return;
    const a = document.createElement("a");
    a.href = cardData.dataUrl;
    a.download = `ju-adhd-${profile.level}.png`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    toast.success(lang === "id" ? "Kartu ADHD berhasil diunduh!" : "ADHD card downloaded!");
  };

  const handleShare = async () => {
    if (!cardData) return;
    const shareText = `My Adult ADHD Screener Result: ${(profile.title as any)[lang] || profile.title.en}\nScreen your focus and dopamine baseline free at: ${window.location.origin}/quiz/adhd-screener`;

    if (navigator.share && navigator.canShare && navigator.canShare({ files: [cardData.file] })) {
      try {
        await navigator.share({
          title: "My Adult ADHD Screener Result",
          text: shareText,
          files: [cardData.file],
        });
      ...
      <DialogContent className="sm:max-w-md max-h-[90vh] flex flex-col p-6 bg-slate-900 border-slate-800 text-slate-100 rounded-3xl overflow-hidden">
        <DialogHeader className="text-left space-y-1">
          <DialogTitle className="text-xl font-bold flex items-center gap-2 text-white">
            ...
            <span>{lang === "id" ? "Bagikan Kartu Hasil ADHD" : "Share Your ADHD Screener Card"}</span>
          </DialogTitle>
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          <button
            onClick={handleShare}
            disabled={!cardData || isGenerating}
            className="flex-1 py-3 px-4 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-slate-950 font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20 transition disabled:opacity-50"
          >
...
export default AdhdShareCardModal;
