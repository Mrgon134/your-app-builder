...
import {
  generateGlossophobiaCard,
  GlossophobiaScoreResult,
  GlossophobiaCardLang,
} from "@/lib/generate-quiz-card";

interface GlossophobiaShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: GlossophobiaScoreResult;
  lang: GlossophobiaCardLang;
}

export const GlossophobiaShareCardModal: React.FC<GlossophobiaShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateGlossophobiaCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Glossophobia card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { dataUrl } = await generateGlossophobiaCard(result, lang);
      const link = document.createElement("a");
      link.download = `nuju-glossophobia-${result.level}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleShare = async () => {
    try {
      const { file } = await generateGlossophobiaCard(result, lang);
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "My Glossophobia & Public Speaking Anxiety Profile (McCroskey PRPSA)",
          text: `I took the Glossophobia Screener on Nuju. My podium panic severity index is ${result.percentage}%. Screen yours free at:`,
          url: "https://nuju.app/quiz/glossophobia",
          files: [file],
        });
      } else {
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/glossophobia");
      setCopied(true);
      ...
      <div className="relative w-full max-w-md bg-neutral-900 border border-amber-800/40 rounded-3xl p-6 shadow-2xl overflow-hidden flex flex-col items-center">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-amber-300">
            Share Story Card · Card #115 (1080x1350)
          </span>
        </div>
        ...
        <p className="text-xs text-neutral-400 text-center mb-4">
          Visualizing McCroskey PRPSA: Stage Fright vs. Scrutiny Terror
        </p>
        ...
        <div className="relative w-full aspect-[4/5] max-h-[360px] rounded-2xl overflow-hidden bg-neutral-950 border border-amber-900/50 shadow-inner flex items-center justify-center mb-5">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Glossophobia Story Card Preview"
        ...
        >
          {copied ? (
            ...
            <>
              ...
              <span>Copy Test URL (nuju.app/quiz/glossophobia)</span>
            </>
          )}
        </button>
      </div>
