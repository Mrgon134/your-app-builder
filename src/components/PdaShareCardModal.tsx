...
import { PdaLang, PdaScoreResult } from "@/data/pda-screener";
import { generatePdaCard } from "@/lib/generate-quiz-card";

interface PdaShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const PdaShareCardModal: React.FC<PdaShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  useEffect(() => {
    ...
    generatePdaCard(result, lang)
      ...
      .catch((err) => {
        console.error("Error generating PDA card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    link.download = `nuju-pda-${result.level}.png`;
    link.click();
    ...
    if (navigator.canShare && navigator.canShare({ files: [cardData.file] })) {
      try {
        await navigator.share({
          files: [cardData.file],
          title: "My PDA & Autonomy Screener Breakdown",
          text: `I took the Pathological Demand Avoidance (PDA) Screener on Nuju. My Profile: ${result.profile.title[lang] || result.profile.title.en} (${result.percentage}% demand friction).`,
        });
      ...
      <DialogContent className="max-w-md bg-neutral-950 text-white border-neutral-800 p-6 rounded-3xl">
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          ...
          <button
            onClick={handleShare}
            disabled={isGenerating || !cardData}
            className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-teal-600 hover:bg-teal-500 text-white font-bold text-xs shadow-lg shadow-teal-600/30 transition disabled:opacity-50"
          >
            {isCopied ? (
              <>
