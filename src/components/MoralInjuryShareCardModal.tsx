...
import {
  generateMoralInjuryCard,
  MoralInjuryScoreResult,
  MoralInjuryCardLang,
} from "@/lib/generate-quiz-card";

interface MoralInjuryShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: MoralInjuryScoreResult;
  lang: MoralInjuryCardLang;
}

export const MoralInjuryShareCardModal: React.FC<MoralInjuryShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateMoralInjuryCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Moral Injury card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { file } = await generateMoralInjuryCard(result, lang);
      const url = URL.createObjectURL(file);
      const a = document.createElement("a");
      a.href = url;
      a.download = `nuju-moral-injury-${result.level}.png`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: "My Moral Injury & Ethical Distress Result | Nuju",
          text: `My Moral Injury Index is ${result.percentage}%. Screen institutional betrayal and moral distress on Nuju:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/moral-injury");
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-3xl p-6 text-white shadow-2xl my-8">
        ...
        <h3 className="text-xl font-bold mb-1">Your Moral Injury Profile Card</h3>
        <p className="text-xs text-neutral-400 mb-5">
          Download or share your clinical assessment profile for Instagram Stories, LinkedIn, or private reflection.
        </p>
        ...
        <div className="flex flex-col gap-2.5">
          <div className="grid grid-cols-2 gap-2.5">
            <button
              onClick={handleShare}
              disabled={generating}
              className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-amber-600 hover:bg-amber-500 font-semibold text-sm transition disabled:opacity-50 text-white"
            >
          ...
          >
            ...
            <span>{copied ? "Link copied to clipboard!" : "Copy test link to share with colleagues"}</span>
          </button>
        </div>
      </div>
    </div>
