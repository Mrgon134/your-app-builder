...
import {
  generateDatingFatigueCard,
  DatingFatigueScoreResult,
  DatingFatigueCardLang,
} from "@/lib/generate-quiz-card";

interface DatingFatigueShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: DatingFatigueScoreResult;
  lang: DatingFatigueCardLang;
}

export const DatingFatigueShareCardModal: React.FC<DatingFatigueShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateDatingFatigueCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Dating Fatigue card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { file } = await generateDatingFatigueCard(result, lang);
      const url = URL.createObjectURL(file);
      const a = document.createElement("a");
      a.href = url;
      a.download = `nuju-dating-fatigue-${result.level}.png`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: "My Dating Fatigue & App Burnout Result | Nuju",
          text: `My Dating App Burnout Index is ${result.percentage}%. Screen dating fatigue and choice paradox on Nuju:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/dating-fatigue");
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-3xl p-6 text-white shadow-2xl my-8">
        ...
        <h3 className="text-xl font-bold mb-1">Your Dating Fatigue Story Card</h3>
        <p className="text-xs text-neutral-400 mb-5">
          Download or share your dating burnout analysis for Instagram Stories, TikTok, or private reflection.
        </p>
        ...
        <div className="relative w-full aspect-[4/5] max-h-[460px] bg-neutral-950 rounded-2xl overflow-hidden border border-neutral-800 mb-6 flex items-center justify-center">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Dating Fatigue Card Preview"
        ...
        <div className="flex flex-col gap-2.5">
          <div className="grid grid-cols-2 gap-2.5">
            <button
              onClick={handleShare}
              disabled={generating}
              className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-purple-600 hover:bg-purple-500 font-semibold text-sm transition disabled:opacity-50 text-white"
            >
          ...
          >
            ...
            <span>{copied ? "Link copied to clipboard!" : "Copy test link to share with single friends"}</span>
          </button>
        </div>
      </div>
    </div>
