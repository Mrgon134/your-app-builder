...
interface PureOShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const PureOShareCardModal: React.FC<PureOShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generatePureOCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Pure O card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { file } = await generatePureOCard(result, lang);
      const url = URL.createObjectURL(file);
      const a = document.createElement("a");
      a.href = url;
      a.download = `nuju-pureo-${result.level}.png`;
    ...
    try {
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "My Pure O & Intrusive Thoughts Result",
          text: `My Intrusive Thought Load: ${result.percentage}% (${result.profile.badge[lang] || result.profile.badge.en}). Screen your intrusive thought pattern:`,
          url: "https://nuju.app/quiz/pure-o",
          files: [file],
        });
      } else {
        handleDownload();
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText("https://nuju.app/quiz/pure-o");
    setCopied(true);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-md rounded-3xl border border-purple-800/40 bg-stone-900 p-6 shadow-2xl text-stone-100">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-bold text-lg text-white">Your Pure O Share Card</h3>
        </div>
        ...
        <div className="relative aspect-[4/5] w-full overflow-hidden rounded-2xl border border-purple-800/40 bg-stone-950 mb-6 flex items-center justify-center">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Pure O Result Card Preview"
        ...
        >
          ...
          <span>{copied ? "Link copied to clipboard!" : "Copy Test Link (nuju.app/quiz/pure-o)"}</span>
        </button>
      </div>
    </div>
  );
};
export default PureOShareCardModal;
