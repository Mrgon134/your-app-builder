...
import { Download, Share2, Check, Loader2, Activity } from "lucide-react";
import { toast } from "sonner";
import { VagalLang } from "@/data/nervous-system";
import { generateNervousSystemCard, VagalCardInput } from "@/lib/generate-quiz-card";

interface NervousSystemShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const NervousSystemShareCardModal: React.FC<NervousSystemShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  useEffect(() => {
    ...
    generateNervousSystemCard(result, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Nervous System card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `ju-nervous-system-${result.dominantState}.png`;
  ...
  const handleNativeShare = async () => {
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [cardData.file] })) {
      try {
        await navigator.share({
          title: "My Polyvagal Nervous System State",
          text: `My autonomic nervous system state: ${result.profile.title[lang]} (${result.profile.badge[lang]}). Check your fight/flight/freeze state on Ju:`,
          url: "https://www.nuju.app/quiz/nervous-system",
          files: [cardData.file],
        });
    ...
    if (navigator.clipboard) {
      try {
        await navigator.clipboard.writeText("https://www.nuju.app/quiz/nervous-system");
        setIsCopied(true);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md bg-card/95 backdrop-blur-xl border border-emerald-500/30 p-6 rounded-3xl shadow-2xl">
        <DialogHeader className="space-y-1">
          ...
          <DialogTitle className="text-xl font-bold tracking-tight text-foreground">
            {lang === "id" ? "Bagikan Peta Sistem Saraf Anda" : "Share Your Nervous System Map"}
          </DialogTitle>
        ...
        <div className="my-3 flex flex-col items-center justify-center">
          {isGenerating ? (
            ...
            <div className="w-full relative group rounded-2xl overflow-hidden shadow-xl border border-emerald-500/20">
              <img
                src={cardData.dataUrl}
                alt="Nervous System Story Share Card"
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            ...
            className="w-full py-3 px-4 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold text-xs flex items-center justify-center gap-2 transition-all shadow-md shadow-emerald-500/20 disabled:opacity-50"
          >
...
export default NervousSystemShareCardModal;
