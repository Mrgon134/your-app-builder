...
interface BfrbShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const BfrbShareCardModal: React.FC<BfrbShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateBfrbCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate BFRB card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { dataUrl } = await generateBfrbCard(result, lang);
      const link = document.createElement("a");
      link.download = `nuju-bfrb-${result.level}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleShare = async () => {
    try {
      const { file } = await generateBfrbCard(result, lang);
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "My Body-Focused Repetitive Behavior Profile (BFRB Screener)",
          text: `I took the Body-Focused Repetitive Behaviors Screener (MGH-SPS / HRT Model) on Nuju. My BFRB compulsion index is ${result.percentage}%. Screen yours free at:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/bfrb");
      setCopied(true);
      ...
      <div className="relative w-full max-w-md bg-neutral-900 border border-rose-800/40 rounded-3xl p-6 shadow-2xl overflow-hidden flex flex-col items-center">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-rose-300">
            Share Story Card · Card #106 (1080x1350)
          </span>
        </div>

        <h3 className="text-xl font-bold text-white text-center mb-1">
          {lang === "id" ? "Kartu Hasil Dorongan BFRB" : "Your BFRB Story Card"}
        </h3>
        ...
        <div className="grid grid-cols-2 gap-3 w-full mb-3">
          ...
          <button
            onClick={handleShare}
            disabled={generating}
            className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-gradient-to-r from-rose-600 to-orange-600 hover:from-rose-500 hover:to-orange-500 text-white text-xs font-semibold transition shadow-md disabled:opacity-50"
          >
