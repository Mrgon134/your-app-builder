...
import { CbtLang } from "@/data/cognitive-distortions";
import { generateCbtCard, CbtCardInput } from "@/lib/generate-quiz-card";

interface CbtShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: CbtCardInput;
  lang: CbtLang;
...
export const CbtShareCardModal: React.FC<CbtShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  useEffect(() => {
    ...
    generateCbtCard(result, lang)
      ...
      .catch((err) => {
        console.error("Error generating CBT card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `ju-cbt-${result.primaryDistortion.id}.png`;
  ...
  const handleNativeShare = async () => {
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [cardData.file] })) {
      try {
        await navigator.share({
          title: "My Dominant Cognitive Distortion Profile",
          text: `My primary thought bias: ${result.primaryDistortion.title[lang]} (${result.primaryDistortion.badge[lang]}). Spot your cognitive distortions on Ju:`,
          url: "https://www.nuju.app/quiz/cognitive-distortions",
          files: [cardData.file],
        });
    ...
    if (navigator.clipboard) {
      try {
        await navigator.clipboard.writeText("https://www.nuju.app/quiz/cognitive-distortions");
        setIsCopied(true);
  ...
  const getTexts = () => {
    switch (lang) {
      case "id":
        return {
          title: "Bagikan Profil Distorsi Kognitif",
      ...
      case "de":
        return {
          title: "Kognitives Verzerrungsprofil teilen",
      ...
      case "fr":
        return {
          title: "Partager votre profil de distorsions",
      ...
      case "es":
        return {
          title: "Compartir perfil de distorsión cognitiva",
      ...
      default:
        return {
          title: "Share Your Cognitive Distortion Profile",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md w-full bg-slate-950/95 border-indigo-900/50 text-white backdrop-blur-xl p-6 rounded-3xl shadow-2xl">
        <DialogHeader className="text-left space-y-1.5">
          <div className="flex items-center gap-2 text-indigo-400 font-semibold text-xs uppercase tracking-wider">
            <Sparkles className="w-3.5 h-3.5" />
            <span>CBT Story Card · 1080 × 1350</span>
          </div>
        ...
        <div className="mt-4 flex flex-col items-center justify-center">
          <div className="relative w-full max-w-[280px] aspect-[4/5] rounded-2xl overflow-hidden border border-indigo-500/20 shadow-xl bg-slate-900 flex items-center justify-center">
            {isGenerating ? (
              <div className="flex flex-col items-center gap-3 p-6 text-center">
                <Loader2 className="w-8 h-8 text-indigo-400 animate-spin" />
                <p className="text-xs text-indigo-200/80">{t.generating}</p>
              </div>
            ) : cardData ? (
              <img
                src={cardData.dataUrl}
                alt="Cognitive Distortion Profile Story Card"
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 w-full py-3 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-medium text-sm transition-all shadow-lg shadow-indigo-600/25 active:scale-[0.98]"
          >
