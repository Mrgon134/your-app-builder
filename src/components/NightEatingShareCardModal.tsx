...
import {
  generateNightEatingCard,
  NightEatingScoreResult,
  NightEatingCardLang,
} from "@/lib/generate-quiz-card";

interface NightEatingShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: NightEatingScoreResult;
  lang: NightEatingCardLang;
}

export const NightEatingShareCardModal: React.FC<NightEatingShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateNightEatingCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Night Eating card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { dataUrl } = await generateNightEatingCard(result, lang);
      const link = document.createElement("a");
      link.download = `nuju-night-eating-${result.level}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleShare = async () => {
    try {
      const { file } = await generateNightEatingCard(result, lang);
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "My Night Eating Syndrome & Circadian Profile (Stunkard NEQ)",
          text: `I took the Night Eating Syndrome Screener on Nuju. My night eating index is ${result.percentage}%. Screen yours free at:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/night-eating");
      setCopied(true);
      ...
      <div className="relative w-full max-w-md bg-neutral-900 border border-indigo-800/40 rounded-3xl p-6 shadow-2xl overflow-hidden flex flex-col items-center">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-amber-300">
            Share Story Card · Card #108 (1080x1350)
          </span>
        </div>

        <h3 className="text-xl font-bold text-white text-center mb-1">
          {lang === "id" ? "Kartu Hasil Night Eating Syndrome" : "Your Night Eating Story Card"}
        </h3>
        ...
        <div className="relative w-full aspect-[4/5] max-h-[380px] rounded-2xl overflow-hidden bg-neutral-950 border border-neutral-800 flex items-center justify-center shadow-inner mb-6">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Night Eating Syndrome Profile Card"
        ...
        <div className="flex items-center gap-3 w-full">
          <button
            ...
            disabled={generating}
            className="flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-amber-500 hover:bg-amber-600 active:scale-95 text-neutral-950 font-bold text-sm shadow-md transition disabled:opacity-50"
          >
