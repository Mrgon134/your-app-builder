...
import { PerfectionismLang, PerfectionismScoreResult } from "@/data/perfectionism";
import { generatePerfectionismCard } from "@/lib/generate-quiz-card";

interface PerfectionismShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const PerfectionismShareCardModal: React.FC<PerfectionismShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  useEffect(() => {
    ...
    generatePerfectionismCard(result, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Perfectionism card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `ju-perfectionism-${result.level}.png`;
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [cardData.file] })) {
      try {
        await navigator.share({
          title: "My Clinical Perfectionism & Fear of Failure Profile",
          text: `My Perfectionism Archetype: ${result.profile.title[lang] || result.profile.title.en} (${result.percentage}%). Screen your perfectionism pattern free:`,
          url: "https://nuju.app/quiz/perfectionism",
          files: [cardData.file],
        });
      } catch (err) {
  ...
  const fallbackCopy = () => {
    navigator.clipboard.writeText("https://nuju.app/quiz/perfectionism");
    setIsCopied(true);
      ...
      <DialogContent className="sm:max-w-md max-w-[92vw] bg-neutral-900 border-neutral-800 text-white p-6 rounded-3xl overflow-hidden shadow-2xl">
        <DialogHeader className="text-left space-y-2">
          ...
          <DialogDescription className="text-xs text-neutral-400">
            Share your perfectionism archetype, fear-of-failure metrics, and cognitive reframe with friends or on social media.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-3 rounded-2xl bg-amber-500 hover:bg-amber-400 text-neutral-950 font-bold text-sm transition shadow-lg shadow-amber-500/20 disabled:opacity-50"
          >
