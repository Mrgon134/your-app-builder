...
import { generateBedtimeProcrastinationCard } from "@/lib/generate-quiz-card";

interface BedtimeProcrastinationShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const BedtimeProcrastinationShareCardModal: React.FC<
  BedtimeProcrastinationShareCardModalProps
> = ({ isOpen, onClose, result, lang }) => {
  ...
  useEffect(() => {
    ...
    generateBedtimeProcrastinationCard(result, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Bedtime Procrastination card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `nuju-bedtime-procrastination-${result.level}.png`;
    ...
    if (
      ...
      try {
        await navigator.share({
          title: "My Revenge Bedtime Procrastination Profile",
          text: `My Sleep Dread Status: ${
            result.profile.title[lang] || result.profile.title.en
          } (${result.percentage}% sleep procrastination index). Test your midnight revenge cycle free:`,
          url: "https://nuju.app/quiz/revenge-bedtime-procrastination",
          files: [cardData.file],
        });
      } catch (err) {
  ...
  const fallbackCopy = () => {
    navigator.clipboard.writeText(
      "https://nuju.app/quiz/revenge-bedtime-procrastination"
    );
    setIsCopied(true);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md max-w-[92vw] bg-neutral-900 border-neutral-800 text-white p-6 rounded-3xl overflow-hidden shadow-2xl">
        <DialogHeader className="text-left space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-semibold w-fit">
            <Sparkles className="w-3.5 h-3.5" />
            <span>Instagram Story Ready (1080x1350)</span>
          </div>
          <DialogTitle className="text-xl font-bold text-white tracking-tight">
            Export Your Sleep Dread Card
          </DialogTitle>
          <DialogDescription className="text-xs text-neutral-400">
            Share your daytime autonomy deficit metrics, dopamine scroll cycle, and nocturnal repair action plan.
          </DialogDescription>
        </DialogHeader>
          ...
          {isGenerating ? (
            ...
            <div className="relative group w-full flex justify-center">
              <img
                ...
                alt="Revenge Bedtime Procrastination Story Card"
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={handleShare}
            disabled={!cardData}
            className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-purple-600 hover:opacity-90 active:scale-[0.98] text-white font-semibold text-sm shadow-lg shadow-amber-500/20 transition-all disabled:opacity-50 disabled:pointer-events-none"
          >
