...
import {
  generateChildhoodEmotionalNeglectCard,
  ChildhoodEmotionalNeglectScoreResult,
  ChildhoodEmotionalNeglectCardLang,
} from "@/lib/generate-quiz-card";

interface ChildhoodEmotionalNeglectShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: ChildhoodEmotionalNeglectScoreResult;
  lang: ChildhoodEmotionalNeglectCardLang;
}

export const ChildhoodEmotionalNeglectShareCardModal: React.FC<ChildhoodEmotionalNeglectShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateChildhoodEmotionalNeglectCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate CEN card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { file } = await generateChildhoodEmotionalNeglectCard(result, lang);
      const url = URL.createObjectURL(file);
      const a = document.createElement("a");
      a.href = url;
      a.download = `nuju-cen-${result.level}.png`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: "My Childhood Emotional Neglect (CEN) Result | Nuju",
          text: `My Emotional Invisibility Index is ${result.percentage}%. Screen childhood emotional neglect and alexithymic emptiness free on Nuju:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/childhood-emotional-neglect");
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-3xl p-6 text-white shadow-2xl my-8">
        ...
        <h3 className="text-xl font-bold mb-1">Your Emotional Invisibility Card</h3>
        <p className="text-xs text-neutral-400 mb-5">
          Download or share your CEN profile for Instagram Stories, TikTok, or gentle self-compassion work.
        </p>
        ...
        <div className="flex flex-col gap-2.5">
          <div className="grid grid-cols-2 gap-2.5">
            <button
              onClick={handleShare}
              disabled={generating}
              className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-rose-500 hover:bg-rose-400 font-semibold text-sm transition disabled:opacity-50 text-neutral-950"
            >
          ...
          >
            ...
            <span>{copied ? "Link copied to clipboard!" : "Copy test link to screen childhood emotional neglect"}</span>
          </button>
        </div>
      </div>
    </div>
