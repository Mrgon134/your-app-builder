...
import { generateAttachmentCompatibilityCard } from "@/lib/generate-quiz-card";

interface AttachmentCompatibilityShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: AttachmentCompatibilityScoreResult;
  lang: AttachmentCompatibilityLang;
}

export const AttachmentCompatibilityShareCardModal: React.FC<
  AttachmentCompatibilityShareCardModalProps
> = ({ isOpen, onClose, result, lang }) => {
  ...
  useEffect(() => {
    ...
    generateAttachmentCompatibilityCard(result, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Attachment Compatibility card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `nuju-attachment-${result.level}.png`;
    ...
    if (
      ...
      try {
        await navigator.share({
          title: "My Attachment Compatibility Profile",
          text: `My Attachment Dynamic: ${
            result.profile.title[lang] || result.profile.title.en
          } (${result.percentage}% relational index). Screen your attachment security:`,
          url: "https://nuju.app/quiz/attachment-compatibility",
          files: [cardData.file],
        });
      } catch (err) {
  ...
  const fallbackCopy = () => {
    navigator.clipboard.writeText("https://nuju.app/quiz/attachment-compatibility");
    setIsCopied(true);
      ...
      <DialogContent className="sm:max-w-md max-w-[92vw] bg-neutral-900 border-neutral-800 text-white p-6 rounded-3xl overflow-hidden shadow-2xl">
        <DialogHeader className="text-left space-y-2">
          ...
          <DialogTitle className="text-xl font-bold text-white tracking-tight">
            Export Your Attachment Card
          </DialogTitle>
          <DialogDescription className="text-xs text-neutral-400">
            Share your pursuit-withdrawal dynamics, security metrics, and de-escalation roadmap.
          </DialogDescription>
        </DialogHeader>
          ...
          {isGenerating ? (
            ...
            <div className="relative group w-full flex justify-center">
              <img
                ...
                alt="Attachment Compatibility Story Card"
