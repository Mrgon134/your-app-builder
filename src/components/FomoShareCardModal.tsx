...
import { generateFomoCard } from "@/lib/generate-quiz-card";

interface FomoShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const FomoShareCardModal: React.FC<
  FomoShareCardModalProps
> = ({ isOpen, onClose, result, lang }) => {
  ...
  useEffect(() => {
    ...
    generateFomoCard(result, lang)
      ...
      .catch((err) => {
        console.error("Error generating FOMO card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `nuju-fomo-${result.level}.png`;
    ...
    if (
      ...
      try {
        await navigator.share({
          title: "My FOMO & Dopamine Profile",
          text: `My Digital Archetype: ${
            result.profile.title[lang] || result.profile.title.en
          } (${result.percentage}% FOMO Index). Check your digital boundary score:`,
          url: "https://nuju.app/quiz/fomo",
          files: [cardData.file],
        });
      } catch (err) {
  ...
  const fallbackCopy = () => {
    navigator.clipboard.writeText("https://nuju.app/quiz/fomo");
    setIsCopied(true);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md max-w-[92vw] bg-neutral-900 border-neutral-800 text-white p-6 rounded-3xl overflow-hidden shadow-2xl">
        <DialogHeader className="text-left space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-300 text-xs font-semibold w-fit">
            <Sparkles className="w-3.5 h-3.5" />
            <span>Instagram Story Ready (1080x1350)</span>
          </div>
          <DialogTitle className="text-xl font-bold text-white tracking-tight">
            Export Your JOMO / FOMO Card
          </DialogTitle>
          <DialogDescription className="text-xs text-neutral-400">
            Share your social comparison metrics, dopamine reflex score, and digital sovereignty roadmap.
          </DialogDescription>
        </DialogHeader>
