...
import {
  generateSpiritualNarcissismCard,
  SpiritualNarcissismScoreResult,
  SpiritualNarcissismCardLang,
} from "@/lib/generate-quiz-card";

interface SpiritualNarcissismShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: SpiritualNarcissismScoreResult;
  lang: SpiritualNarcissismCardLang;
}

export const SpiritualNarcissismShareCardModal: React.FC<SpiritualNarcissismShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateSpiritualNarcissismCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Spiritual Narcissism card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { file } = await generateSpiritualNarcissismCard(result, lang);
      const url = URL.createObjectURL(file);
      const a = document.createElement("a");
      a.href = url;
      a.download = `nuju-spiritual-narcissism-${result.level}.png`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: "My Spiritual Narcissism & Superiority Result | Nuju",
          text: `My Spiritual Ego Index is ${result.percentage}%. Screen enlightened gaslighting and spiritual pride on Nuju:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/spiritual-narcissism");
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-3xl p-6 text-white shadow-2xl my-8">
        ...
        <h3 className="text-xl font-bold mb-1">Your Spiritual Ego Card</h3>
        <p className="text-xs text-neutral-400 mb-5">
          Download or share your spiritual ego profile for Instagram Stories, TikTok, or grounded shadow work.
        </p>
        ...
        <div className="relative w-full aspect-[4/5] max-h-[460px] bg-neutral-950 rounded-2xl overflow-hidden border border-neutral-800 mb-6 flex items-center justify-center">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Spiritual Narcissism Card Preview"
        ...
        <div className="flex flex-col gap-2.5">
          <div className="grid grid-cols-2 gap-2.5">
            <button
              onClick={handleShare}
              disabled={generating}
              className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-amber-500 hover:bg-amber-400 font-semibold text-sm transition disabled:opacity-50 text-neutral-950"
            >
          ...
          >
            ...
            <span>{copied ? "Link copied to clipboard!" : "Copy test link to unmask the spiritual ego"}</span>
          </button>
        </div>
      </div>
    </div>
