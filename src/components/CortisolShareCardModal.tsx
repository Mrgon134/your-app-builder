...
import { CortisolLang, CortisolScoreResult } from "@/data/cortisol";
import { generateCortisolCard } from "@/lib/generate-quiz-card";

interface CortisolShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const CortisolShareCardModal: React.FC<CortisolShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  useEffect(() => {
    ...
    generateCortisolCard(result, lang)
      .then((generated) => {
        if (isMounted) {
      ...
      .catch((err) => {
        console.error("Error generating Cortisol card:", err);
        if (isMounted) {
          setIsGenerating(false);
  ...
  const handleDownload = () => {
    ...
    a.download = `nuju-cortisol-${result.level}.png`;
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [cardData.file] })) {
      try {
        await navigator.share({
          title: "My Cortisol & Somatic Stress Profile",
          text: `My Somatic Load Status: ${result.profile.title[lang] || result.profile.title.en} (${result.percentage}% somatic burden). Test your nervous system dysregulation free:`,
          url: "https://nuju.app/quiz/cortisol-stress",
          files: [cardData.file],
        });
      } catch (err) {
  ...
  const fallbackCopy = () => {
    navigator.clipboard.writeText("https://nuju.app/quiz/cortisol-stress");
    setIsCopied(true);
      ...
      <DialogContent className="sm:max-w-md max-w-[92vw] bg-neutral-900 border-neutral-800 text-white p-6 rounded-3xl overflow-hidden shadow-2xl">
        <DialogHeader className="text-left space-y-2">
          ...
          <DialogTitle className="text-xl font-bold text-white tracking-tight">
            Export Your Somatic Health Card
          </DialogTitle>
          <DialogDescription className="text-xs text-neutral-400">
            Share your cortisol dysregulation markers, physiological load indicators, and nervous system reset protocol.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="mt-4 flex flex-col items-center justify-center">
          {isGenerating ? (
            <div className="w-full aspect-[4/5] max-h-[380px] rounded-2xl bg-neutral-950 border border-neutral-800 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-xs text-neutral-400 font-medium">Synthesizing somatic profile card...</p>
            </div>
          ) : cardData ? (
            <div className="relative group w-full flex justify-center">
              <img
                src={cardData.dataUrl}
                alt="Cortisol Stress Story Card"
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={handleShare}
            disabled={!cardData}
            className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-r from-teal-500 to-emerald-600 hover:opacity-90 active:scale-[0.98] text-white font-semibold text-sm shadow-lg shadow-teal-500/20 transition-all disabled:opacity-50 disabled:pointer-events-none"
          >
            {isCopied ? <Check className="w-4 h-4 text-teal-200" /> : <Share2 className="w-4 h-4" />}
