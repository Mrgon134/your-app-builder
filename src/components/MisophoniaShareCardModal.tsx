...
import {
  generateMisophoniaCard,
  MisophoniaScoreResult,
  MisophoniaCardLang,
} from "@/lib/generate-quiz-card";

interface MisophoniaShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: MisophoniaScoreResult;
  lang: MisophoniaCardLang;
}

export const MisophoniaShareCardModal: React.FC<MisophoniaShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateMisophoniaCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Misophonia card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { dataUrl } = await generateMisophoniaCard(result, lang);
      const link = document.createElement("a");
      link.download = `nuju-misophonia-${result.level}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleShare = async () => {
    try {
      const { file } = await generateMisophoniaCard(result, lang);
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "My Misophonia & Sound Sensitivity Profile",
          text: `I took the Misophonia & Sound Rage Screener on Nuju. My misophonic reactivity index is ${result.percentage}%. Screen your sound triggers at:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/misophonia");
      setCopied(true);
      ...
      <div className="relative w-full max-w-md bg-neutral-900 border border-rose-800/40 rounded-3xl p-6 shadow-2xl overflow-hidden flex flex-col items-center">
        ...
        <div className="w-full max-h-[440px] aspect-[4/5] bg-black/50 rounded-2xl border border-white/10 overflow-hidden flex items-center justify-center relative mb-6">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Misophonia Story Card Preview"
