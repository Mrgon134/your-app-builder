...
import {
  generateRuminationCard,
  RuminationScoreResult,
  RuminationCardLang,
} from "@/lib/generate-quiz-card";

interface RuminationShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: RuminationScoreResult;
  lang: RuminationCardLang;
}

export const RuminationShareCardModal: React.FC<RuminationShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateRuminationCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Rumination card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { dataUrl } = await generateRuminationCard(result, lang);
      const link = document.createElement("a");
      link.download = `nuju-rumination-${result.level}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleShare = async () => {
    try {
      const { file } = await generateRuminationCard(result, lang);
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "My Depressive Rumination Profile (Nolen-Hoeksema RRS)",
          text: `I took the Depressive Rumination & Brooding Loop Screener on Nuju. My rumination index is ${result.percentage}%. Screen yours free at:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/rumination");
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
      <div className="relative w-full max-w-md bg-neutral-900 border border-violet-800/40 rounded-3xl p-6 shadow-2xl overflow-hidden flex flex-col items-center">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-violet-300">
            Share Story Card · Card #104 (1080x1350)
          </span>
        </div>

        <h3 className="text-xl font-bold text-white text-center mb-1">
          {lang === "id" ? "Kartu Hasil Ruminasi & Overthinking" : "Your Rumination Story Card"}
        </h3>
        ...
        <div className="relative w-full aspect-[4/5] max-h-[380px] rounded-2xl overflow-hidden bg-neutral-950 border border-neutral-800 flex items-center justify-center shadow-inner mb-6">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Rumination Story Card Preview"
        ...
        <div className="grid grid-cols-2 gap-3 w-full mb-3">
          ...
          <button
            onClick={handleShare}
            disabled={generating}
            className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white text-xs font-semibold transition shadow-md disabled:opacity-50"
          >
