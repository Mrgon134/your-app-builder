...
import {
  generateSensorimotorCard,
  SensorimotorScoreResult,
  SensorimotorCardLang,
} from "@/lib/generate-quiz-card";

interface SensorimotorShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: SensorimotorScoreResult;
  lang: SensorimotorCardLang;
}

export const SensorimotorShareCardModal: React.FC<SensorimotorShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateSensorimotorCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Sensorimotor card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { file } = await generateSensorimotorCard(result, lang);
      const url = URL.createObjectURL(file);
      const a = document.createElement("a");
      a.href = url;
      a.download = `nuju-sensorimotor-${result.level}.png`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: "My Sensorimotor OCD & Somatic Hyperawareness Profile | Nuju",
          text: `My Sensorimotor Hyperawareness load is ${result.percentage}%. Screen breathing & swallowing fixation on Nuju:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/sensorimotor-ocd");
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-3xl p-6 text-white shadow-2xl my-8">
        ...
        <h3 className="text-xl font-bold mb-1">Your Biological Autopilot Profile Card</h3>
        <p className="text-xs text-neutral-400 mb-5">
          Download or share your clinical Sensorimotor OCD profile to normalize manual breathing dread and release somatic hyper-focus.
        </p>
        ...
        <div className="flex flex-col gap-2.5">
          <div className="grid grid-cols-2 gap-2.5">
            <button
              onClick={handleShare}
              disabled={generating}
              className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-cyan-600 hover:bg-cyan-500 font-semibold text-sm transition disabled:opacity-50 text-neutral-950 font-bold shadow-lg shadow-cyan-950/40"
            >
