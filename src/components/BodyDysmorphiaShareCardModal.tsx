...
import {
  generateBodyDysmorphiaCard,
  BodyDysmorphiaScoreResult,
  BodyDysmorphiaCardLang,
} from "@/lib/generate-quiz-card";

interface BodyDysmorphiaShareCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  result: BodyDysmorphiaScoreResult;
  lang: BodyDysmorphiaCardLang;
}

export const BodyDysmorphiaShareCardModal: React.FC<BodyDysmorphiaShareCardModalProps> = ({
  isOpen,
  onClose,
  result,
  lang,
  ...
  React.useEffect(() => {
    if (isOpen) {
      setGenerating(true);
      generateBodyDysmorphiaCard(result, lang)
        .then(({ dataUrl }) => {
          setCardPreview(dataUrl);
        })
        .catch((err) => {
          console.error("Failed to generate Body Dysmorphia card:", err);
        })
        .finally(() => {
          setGenerating(false);
        });
  ...
  const handleDownload = async () => {
    try {
      const { file } = await generateBodyDysmorphiaCard(result, lang);
      const url = URL.createObjectURL(file);
      const a = document.createElement("a");
      a.href = url;
      a.download = `nuju-body-dysmorphia-${result.level}.png`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: "My Body Dysmorphia (BDD) Profile | Nuju",
          text: `My Body Dysmorphic Friction index is ${result.percentage}%. Screen mirror checking & flaw preoccupation on Nuju:`,
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText("https://nuju.app/quiz/body-dysmorphia");
      setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-3xl p-6 text-white shadow-2xl my-8">
        ...
        <h3 className="text-xl font-bold mb-1">Your Body Image Reflection Card</h3>
        <p className="text-xs text-neutral-400 mb-5">
          Download or share your clinical BDD & mirror checking profile to normalize body neutrality and reduce visual shame.
        </p>
        ...
        <div className="relative w-full aspect-[4/5] max-h-[460px] bg-neutral-950 rounded-2xl overflow-hidden border border-neutral-800 mb-6 flex items-center justify-center">
          {generating ? (
            ...
            <img
              src={cardPreview}
              alt="Body Dysmorphia Card Preview"
