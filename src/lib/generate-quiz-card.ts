...
export interface DatingFatigueScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    swipe_apathy: { score: number; percentage: number };
    paradox_of_choice: { score: number; percentage: number };
    rejection_desensitization: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Dating Fatigue & Paradox of Choice Screener (Barry Schwartz & Eli Finkel model).
 */
export async function generateDatingFatigueCard(
  result: DatingFatigueScoreResult,
  lang: DatingFatigueCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Cyber Violet & Midnight Indigo
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#080417");
  bgGrad.addColorStop(0.5, "#16082F");
  ...
  const glow2 = ctx.createRadialGradient(850, 950, 20, 850, 950, 480);
  glow2.addColorStop(0, "rgba(236, 72, 153, 0.20)");
  ...
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "2px";
  ctx.fillText("NUJU CLINICAL SCREENER · DATING PARADOX MODEL", 80, 100);

  // Primary Title
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "900 46px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "-0.5px";
  ctx.fillText("DATING FATIGUE PROFILE", 80, 165);

  // Subtitle / Framework
  ctx.fillStyle = "#C084FC";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "0.5px";
  ctx.fillText("Dr. Barry Schwartz & Dr. Eli Finkel Relational Choice Architecture", 80, 205);
  ...
  const badgeGradient = ctx.createLinearGradient(120, cardY + 35, 120 + badgeWidth, cardY + 35);
  badgeGradient.addColorStop(0, "#A855F7");
  badgeGradient.addColorStop(1, "#7E22CE");
  ctx.fillStyle = badgeGradient;
  drawRoundedRect(ctx, 120, cardY + 28, badgeWidth, 38, 19);
  ctx.fill();
  ...
  ctx.fillStyle = "#E9D5FF";
  ctx.font = "bold 24px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`App Burnout Index: ${result.percentage}%`, 330, cardY + 155);
  ...
  ctx.fillStyle = "#E2E8F0";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("ROMANTIC EXHAUSTION & CHOICE PARADOX", 120, metricY + 50);

  // 3 Subscale Progress Bars
  const subscales = [
    { label: "Swipe Apathy & Fatigue", val: result.subscales.swipe_apathy.percentage },
    { label: "Paradox of Choice Paralysis", val: result.subscales.paradox_of_choice.percentage },
    { label: "Rejection Desensitization", val: result.subscales.rejection_desensitization.percentage },
  ];
  ...
  ctx.font = "20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(
    "Dr. Barry Schwartz Insight: When choices seem endless, satisfaction plummets and self-blame skyrockets.",
    120,
    metricY + 235
  );
  ...
  ctx.fillStyle = "#F8FAFC";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("SLOW DATING & DOPAMINE RE-REGULATION PROTOCOL", 120, 975);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Assess your dating fatigue & app burnout free at:", 80, 1200);

  ctx.fillStyle = "#C084FC";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/dating-fatigue", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-dating-fatigue-${result.level}.png`, { type: "image/png" });
...
export interface ToxicPositivityScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    suppression_shaming: { score: number; percentage: number };
    spiritual_bypassing: { score: number; percentage: number };
    emotional_isolation: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Toxic Positivity & Emotional Invalidation Screener (Susan David model).
 */
export async function generateToxicPositivityCard(
  result: ToxicPositivityScoreResult,
  lang: ToxicPositivityCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Obsidian Slate & Electric Citron
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#060E0A");
  bgGrad.addColorStop(0.5, "#0E1F16");
  bgGrad.addColorStop(1, "#070E0A");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, width, height);

  // Soft glowing atmospheric orbs
  const glow1 = ctx.createRadialGradient(250, 270, 20, 250, 270, 500);
  glow1.addColorStop(0, "rgba(163, 230, 53, 0.22)");
  glow1.addColorStop(1, "rgba(163, 230, 53, 0)");
  ctx.fillStyle = glow1;
  ctx.fillRect(0, 0, width, height);

  const glow2 = ctx.createRadialGradient(850, 950, 20, 850, 950, 480);
  glow2.addColorStop(0, "rgba(34, 197, 94, 0.18)");
  ...
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "2px";
  ctx.fillText("NUJU CLINICAL SCREENER · EMOTIONAL AGILITY MODEL", 80, 100);

  // Primary Title
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "900 46px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "-0.5px";
  ctx.fillText("TOXIC POSITIVITY PROFILE", 80, 165);

  // Subtitle / Framework
  ctx.fillStyle = "#A3E635";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "0.5px";
  ctx.fillText("Dr. Susan David & Whitney Goodman Emotional Validation Architecture", 80, 205);
  ...
  badgeGradient.addColorStop(1, "#65A30D");
  ctx.fillStyle = badgeGradient;
  drawRoundedRect(ctx, 120, cardY + 28, badgeWidth, 38, 19);
  ctx.fill();
  ...
  ctx.font = "bold 24px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`Invalidation Index: ${result.percentage}%`, 330, cardY + 155);
  ...
  ctx.fillStyle = "#E2E8F0";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("EMOTIONAL SUPPRESSION & BYPASSING", 120, metricY + 50);

  // 3 Subscale Progress Bars
  const subscales = [
    { label: "Suppression Shaming", val: result.subscales.suppression_shaming.percentage },
    { label: "Spiritual Bypassing", val: result.subscales.spiritual_bypassing.percentage },
    { label: "Emotional Isolation", val: result.subscales.emotional_isolation.percentage },
  ];
  ...
  ctx.font = "20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(
    "Dr. Susan David Insight: Discomfort is the price of admission to a meaningful life. Positivity without truth is tyranny.",
    120,
    metricY + 235
  );
  ...
  ctx.fillStyle = "#F8FAFC";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("EMOTIONAL AGILITY & AUTHENTIC FEELING PROTOCOL", 120, 975);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Assess your emotional invalidation & forced optimism free at:", 80, 1200);

  ctx.fillStyle = "#A3E635";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/toxic-positivity", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-toxic-positivity-${result.level}.png`, { type: "image/png" });
...
export interface ChronicResentmentScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    injustice_rumination_replay: { score: number; percentage: number };
    somatic_bitterness_poisoning: { score: number; percentage: number };
    hostility_defense_hardening: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Chronic Resentment & Grudge Holding Screener (Stanford Forgiveness model).
 */
export async function generateChronicResentmentCard(
  result: ChronicResentmentScoreResult,
  lang: ChronicResentmentCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Crimson Ember & Charcoal Obsidian
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#0A0404");
  bgGrad.addColorStop(0.5, "#1B0808");
  ...
  // Atmospheric glows
  const glow1 = ctx.createRadialGradient(250, 270, 20, 250, 270, 500);
  glow1.addColorStop(0, "rgba(239, 68, 68, 0.22)");
  ...
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "2px";
  ctx.fillText("NUJU CLINICAL SCREENER · STANFORD FORGIVENESS ARCHITECTURE", 80, 100);

  // Primary Title
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "900 44px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "-0.5px";
  ctx.fillText("CHRONIC RESENTMENT PROFILE", 80, 165);

  // Subtitle / Framework
  ctx.fillStyle = "#F87171";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "0.5px";
  ctx.fillText("Dr. Fred Luskin (Stanford Forgiveness) & Dr. Robert Enright Model", 80, 205);
  ...
  badgeGradient.addColorStop(1, "#991B1B");
  ctx.fillStyle = badgeGradient;
  drawRoundedRect(ctx, 120, cardY + 28, badgeWidth, 38, 19);
  ctx.fill();
  ...
  ctx.font = "bold 24px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`Bitterness Index: ${result.percentage}%`, 330, cardY + 155);
  ...
  ctx.fillStyle = "#E2E8F0";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("INJUSTICE RUMINATION & SOMATIC BITTERNESS METRICS", 120, metricY + 50);

  // 3 Subscale Progress Bars
  const subscales = [
    { label: "Injustice Rumination & Replay", val: result.subscales.injustice_rumination_replay.percentage },
    { label: "Somatic Bitterness & Poisoning", val: result.subscales.somatic_bitterness_poisoning.percentage },
    { label: "Hostility & Defensive Hardening", val: result.subscales.hostility_defense_hardening.percentage },
  ];
  ...
  ctx.font = "20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(
    "Fred Luskin Insight: Resentment is the emotional equivalent of holding hot coals waiting to throw them at another.",
    120,
    metricY + 235
  );
  ...
  ctx.fillStyle = "#F8FAFC";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("SOMATIC BITTERNESS PURGE & SOVEREIGN BOUNDARY PROTOCOL", 120, 975);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Assess your chronic resentment & grudge burden free at:", 80, 1200);

  ctx.fillStyle = "#F87171";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/chronic-resentment", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-resentment-${result.level}.png`, { type: "image/png" });
...
export interface PathologicalAltruismScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    destructive_rescuing_compulsion: { score: number; percentage: number };
    enabler_boundary_erosion: { score: number; percentage: number };
    martyrdom_depletion: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Pathological Altruism & Savior Complex Screener (Dr. Barbara Oakley model).
 */
export async function generatePathologicalAltruismCard(
  result: PathologicalAltruismScoreResult,
  lang: PathologicalAltruismCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Amethyst & Soft Gold
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#08060F");
  bgGrad.addColorStop(0.5, "#160F2B");
  ...
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "2px";
  ctx.fillText("NUJU CLINICAL SCREENER · PATHOLOGICAL ALTRUISM & ENABLING", 80, 100);

  // Primary Title
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "900 44px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "-0.5px";
  ctx.fillText("PATHOLOGICAL ALTRUISM PROFILE", 80, 165);

  // Subtitle / Framework
  ctx.fillStyle = "#C084FC";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.letterSpacing = "0.5px";
  ctx.fillText("Dr. Barbara Oakley & Dr. Rachel Naomi Remen Caretaker Model", 80, 205);
  ...
  const badgeWidth = ctx.measureText(badgeText).width + 48;
  const badgeGradient = ctx.createLinearGradient(120, cardY + 35, 120 + badgeWidth, cardY + 35);
  badgeGradient.addColorStop(0, "#9333EA");
  badgeGradient.addColorStop(1, "#6B21A8");
  ctx.fillStyle = badgeGradient;
  drawRoundedRect(ctx, 120, cardY + 28, badgeWidth, 38, 19);
  ctx.fill();
  ...
  ctx.fillStyle = "#E9D5FF";
  ctx.font = "bold 24px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`Self-Sacrifice Index: ${result.percentage}%`, 330, cardY + 155);
  ...
  ctx.fillStyle = "#E2E8F0";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("COMPULSIVE RESCUING & ENABLING METRICS", 120, metricY + 50);

  // 3 Subscale Progress Bars
  const subscales = [
    { label: "Destructive Rescuing", val: result.subscales.destructive_rescuing_compulsion.percentage },
    { label: "Enabler Boundary Erosion", val: result.subscales.enabler_boundary_erosion.percentage },
    { label: "Martyrdom Depletion", val: result.subscales.martyrdom_depletion.percentage },
  ];
  ...
  ctx.font = "20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(
    "Barbara Oakley Insight: Altruism becomes pathological when helping destroys the giver and enables stagnation.",
    120,
    metricY + 235
  );
  ...
  ctx.fillStyle = "#F8FAFC";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("ETHICAL SELF-PRIORITIZATION & DISENGAGEMENT PROTOCOL", 120, 975);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Assess your pathological altruism & savior trap free at:", 80, 1200);

  ctx.fillStyle = "#C084FC";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/pathological-altruism", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-altruism-${result.level}.png`, { type: "image/png" });
...
export interface ThanatophobiaScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    existential_annihilation_dread: { score: number; percentage: number };
    symbolic_immortality_striving: { score: number; percentage: number };
    mortality_salience_paralysis: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Thanatophobia & Existential Death Anxiety Screener (Dr. Irvin Yalom & TMT Model).
 */
export async function generateThanatophobiaCard(
  result: ThanatophobiaScoreResult,
  lang: ThanatophobiaCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Obsidian & Cosmic Violet
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#07050A");
  bgGrad.addColorStop(0.5, "#140E20");
  bgGrad.addColorStop(1, "#090610");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, width, height);

  // Subtle Cosmic Radiant Gold / Violet Accent
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.25, 40, width * 0.8, height * 0.25, 550);
  radial.addColorStop(0, "rgba(250, 204, 21, 0.12)");
  radial.addColorStop(1, "rgba(250, 204, 21, 0)");
  ctx.fillStyle = radial;
  ctx.fillRect(0, 0, width, height);
  ...
  ctx.fillStyle = "#FDE047";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · THANATOPHOBIA", 108, 115);

  // Category Tag
  ctx.fillStyle = "#C084FC";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("EXISTENTIAL DEATH ANXIETY (YALOM MODEL)", 80, 185);
  ...
  // Badge inside card
  const badgeText = (result.profile.badge[lang] || result.profile.badge.en || "EXISTENTIAL ALARM").toUpperCase();
  ...
  // Title
  const titleText = result.profile.title[lang] || result.profile.title.en || "Death Anxiety Index";
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "900 46px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const titleLines = wrapText(ctx, titleText, width - 240);
  let curY = cardBoxY + 150;
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.6)";
  ctx.font = "600 24px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`Mortality Salience (${result.score}/36 pts)`, 350, curY + 25);

  ctx.fillStyle = "#C084FC";
  ctx.font = "500 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Annihilation & Finitude Tension", 350, curY + 60);

  // Subscale Progress Bars
  curY += 120;
  const subscales = [
    { label: "Annihilation Dread", pct: result.subscales.existential_annihilation_dread.percentage, color: "#FACC15" },
    { label: "Immortality Striving", pct: result.subscales.symbolic_immortality_striving.percentage, color: "#E879F9" },
    { label: "Mortality Paralysis", pct: result.subscales.mortality_salience_paralysis.percentage, color: "#FDE047" },
  ];
  ...
  ctx.fillStyle = "#FDE047";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. IRVIN D. YALOM CLINICAL RULE:", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Though the physicality of death destroys man, the idea of death saves him. When we confront mortality, life is elevated into profound urgency."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen your thanatophobia & death anxiety free at:", 80, 1200);

  ctx.fillStyle = "#FACC15";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/thanatophobia", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-thanatophobia-${result.level}.png`, { type: "image/png" });
...
export interface ExecutiveDysfunctionScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    task_initiation_activation_barrier: { score: number; percentage: number };
    working_memory_object_impermanence: { score: number; percentage: number };
    time_blindness_urgency_dependence: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Executive Dysfunction & Task Paralysis Screener (Dr. Russell Barkley & Dr. Thomas Brown Model).
 */
export async function generateExecutiveDysfunctionCard(
  result: ExecutiveDysfunctionScoreResult,
  lang: ExecutiveDysfunctionCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Electric Indigo & Midnight Cyber
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#060714");
  bgGrad.addColorStop(0.5, "#10132B");
  bgGrad.addColorStop(1, "#090A1A");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, width, height);

  // Subtle Cyan & Amber Radial Accent
  const radial = ctx.createRadialGradient(width * 0.85, height * 0.25, 40, width * 0.85, height * 0.25, 550);
  radial.addColorStop(0, "rgba(56, 189, 248, 0.14)");
  radial.addColorStop(1, "rgba(56, 189, 248, 0)");
  ctx.fillStyle = radial;
  ctx.fillRect(0, 0, width, height);
  ...
  ctx.fillStyle = "#38BDF8";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · EXECUTIVE FUNCTION", 108, 115);

  // Category Tag
  ctx.fillStyle = "#F59E0B";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("TASK PARALYSIS & WORKING MEMORY (BARKLEY MODEL)", 80, 185);
  ...
  // Badge inside card
  const badgeText = (result.profile.badge[lang] || result.profile.badge.en || "EXECUTIVE STRAIN").toUpperCase();
  ctx.fillStyle = "rgba(56, 189, 248, 0.15)";
  ctx.beginPath();
  ctx.roundRect(120, cardBoxY + 45, 340, 48, 24);
  ctx.fill();
  ...
  // Title
  const titleText = result.profile.title[lang] || result.profile.title.en || "Executive Activation Dynamic";
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "900 46px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const titleLines = wrapText(ctx, titleText, width - 240);
  let curY = cardBoxY + 150;
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.6)";
  ctx.font = "600 24px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`Executive Friction (${result.score}/36 pts)`, 350, curY + 25);

  ctx.fillStyle = "#F59E0B";
  ctx.font = "500 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Prefrontal Intention-to-Action Gap", 350, curY + 60);

  // Subscale Progress Bars
  curY += 120;
  const subscales = [
    { label: "Activation Barrier", pct: result.subscales.task_initiation_activation_barrier.percentage, color: "#38BDF8" },
    { label: "Working Memory", pct: result.subscales.working_memory_object_impermanence.percentage, color: "#818CF8" },
    { label: "Time Blindness", pct: result.subscales.time_blindness_urgency_dependence.percentage, color: "#F59E0B" },
  ];
  ...
  ctx.fillStyle = "#38BDF8";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. RUSSELL BARKLEY CLINICAL PRINCIPLE:", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Executive dysfunction is not an issue of knowing what to do, but of doing what you know. It is a neurological failure to bring knowledge to bear at the point of performance."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen your executive dysfunction & ADHD paralysis at:", 80, 1200);

  ctx.fillStyle = "#38BDF8";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/executive-dysfunction", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-executive-${result.level}.png`, { type: "image/png" });
...
export interface SensorimotorScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    sensorimotor_fixation: { score: number; percentage: number };
    loss_of_autopilot_dread: { score: number; percentage: number };
    compulsive_distraction_strain: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Sensorimotor OCD & Somatic Hyperawareness Screener (Dr. David Veale Model).
 */
export async function generateSensorimotorCard(
  result: SensorimotorScoreResult,
  lang: SensorimotorCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Ocean Cyan & Dark Obsidian
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#030E14");
  bgGrad.addColorStop(0.5, "#061A24");
  ...
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.25, 40, width * 0.8, height * 0.25, 550);
  radial.addColorStop(0, "rgba(6, 182, 212, 0.15)");
  ...
  ctx.fillStyle = "#67E8F9";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · SENSORIMOTOR OCD", 108, 115);

  // Category Tag
  ctx.fillStyle = "#22D3EE";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("SOMATOSENSORY HYPERAWARENESS (VEALE MODEL)", 80, 185);
  ...
  // Badge inside card
  const badgeText = (result.profile.badge[lang] || result.profile.badge.en || "HYPERAWARENESS").toUpperCase();
  ...
  // Title
  const titleText = result.profile.title[lang] || result.profile.title.en || "Sensorimotor Profile";
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "900 44px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const titleLines = wrapText(ctx, titleText, width - 240);
  let curY = cardBoxY + 150;
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.6)";
  ctx.font = "600 24px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`Hyperawareness Load (${result.score}/36 pts)`, 350, curY + 25);

  ctx.fillStyle = "#67E8F9";
  ctx.font = "500 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Biological Autopilot Friction", 350, curY + 60);

  // Subscale Progress Bars
  curY += 120;
  const subscales = [
    { label: "Sensorimotor Lock-In", pct: result.subscales.sensorimotor_fixation.percentage, color: "#22D3EE" },
    { label: "Loss of Autopilot Dread", pct: result.subscales.loss_of_autopilot_dread.percentage, color: "#67E8F9" },
    { label: "Compulsive Distraction Strain", pct: result.subscales.compulsive_distraction_strain.percentage, color: "#06B6D4" },
  ];
  ...
  ctx.fillStyle = "#67E8F9";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. DAVID VEALE CLINICAL PRINCIPLE:", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Sensorimotor OCD is not a failure of involuntary biological organs; it is an attentional lock-in trap where fighting the sensation convinces the brain that natural breathing is dangerous."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen your sensorimotor OCD & breathing focus at:", 80, 1200);

  ctx.fillStyle = "#22D3EE";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/sensorimotor-ocd", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-sensorimotor-${result.level}.png`, { type: "image/png" });

  return { dataUrl, blob, file };
}

export type VulnerabilityHangoverCardLang = "en" | "id" | "de" | "fr" | "es";

export interface VulnerabilityHangoverScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    post_social_forensic_replay: { score: number; percentage: number };
    exposure_nausea_shame: { score: number; percentage: number };
    retraction_isolation_urge: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Vulnerability Hangover & Post-Social Exposure Dread Screener (Dr. Brené Brown Model).
 */
export async function generateVulnerabilityHangoverCard(
  result: VulnerabilityHangoverScoreResult,
  lang: VulnerabilityHangoverCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Velvet Indigo & Dark Obsidian
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#080616");
  bgGrad.addColorStop(0.5, "#120D2C");
  bgGrad.addColorStop(1, "#060412");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, width, height);

  // Subtle Indigo / Violet Glow Radial
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.25, 40, width * 0.8, height * 0.25, 550);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.08)";
  ctx.beginPath();
  ctx.roundRect(80, 80, 540, 56, 28);
  ctx.fill();

  ctx.fillStyle = "#A5B4FC";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · VULNERABILITY HANGOVER", 108, 115);

  // Category Tag
  ctx.fillStyle = "#818CF8";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("POST-SOCIAL EXPOSURE DREAD (BRENÉ BROWN MODEL)", 80, 185);
  ...
  // Badge inside card
  const badgeText = (result.profile.badge[lang] || result.profile.badge.en || "EXPOSURE SHAME").toUpperCase();
  ctx.fillStyle = "rgba(99, 102, 241, 0.18)";
  ctx.beginPath();
  ctx.roundRect(120, cardBoxY + 45, 340, 48, 24);
  ctx.fill();
  ...
  // Title
  const titleText = result.profile.title[lang] || result.profile.title.en || "Vulnerability Profile";
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "900 44px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const titleLines = wrapText(ctx, titleText, width - 240);
  let curY = cardBoxY + 150;
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.6)";
  ctx.font = "600 24px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`Post-Social Friction (${result.score}/36 pts)`, 350, curY + 25);

  ctx.fillStyle = "#A5B4FC";
  ctx.font = "500 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Exposure Nausea & Replay Load", 350, curY + 60);

  // Subscale Progress Bars
  curY += 120;
  const subscales = [
    { label: "Forensic Conversation Replay", pct: result.subscales.post_social_forensic_replay.percentage, color: "#818CF8" },
    { label: "Exposure Nausea & Shame", pct: result.subscales.exposure_nausea_shame.percentage, color: "#A5B4FC" },
    { label: "Retraction & Ghosting Urge", pct: result.subscales.retraction_isolation_urge.percentage, color: "#6366F1" },
  ];
  ...
  ctx.fillStyle = "#A5B4FC";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. BRENÉ BROWN CLINICAL PRINCIPLE:", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"A vulnerability hangover is not a sign that you made a mistake; it is the visceral flutter of having dared to show up authentically when you cannot control the outcome."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen your vulnerability hangover & social dread at:", 80, 1200);

  ctx.fillStyle = "#818CF8";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/vulnerability-hangover", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-vulnerability-hangover-${result.level}.png`, { type: "image/png" });
...
// ============================================================================
// BATCH 59: AGORAPHOBIA & COVERT NARCISSISM SCREENERS
// ============================================================================

export type AgoraphobiaCardLang = "en" | "id" | "de" | "fr" | "es";

export interface AgoraphobiaScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    spatial_escape_entrapment: { score: number; percentage: number };
    fear_of_fear_interoception: { score: number; percentage: number };
    safety_tether_constriction: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Agoraphobia & Panic Escape Screener (Dr. Borwin Bandelow & Dr. David Barlow Model).
 */
export async function generateAgoraphobiaCard(
  result: AgoraphobiaScoreResult,
  lang: AgoraphobiaCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Warm Sandstone Charcoal
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#130902");
  bgGrad.addColorStop(0.5, "#221104");
  bgGrad.addColorStop(1, "#0B0501");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, width, height);

  // Warm Amber Glow Radial
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.25, 40, width * 0.8, height * 0.25, 550);
  ...
  ctx.fillStyle = "#FCD34D";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · AGORAPHOBIA SCREENER", 108, 115);

  // Category Tag
  ctx.fillStyle = "#F59E0B";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("PANIC & AGORAPHOBIA SCALE (PAS / BARLOW)", 80, 185);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Spatial Avoidance Load", 350, curY + 48);

  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "16px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`Clinical PAS Score: ${result.score} / 36 pts`, 350, curY + 78);

  // Subscales
  curY += 145;
  const subscales = [
    { label: "Spatial Escape & Entrapment Dread", pct: result.subscales.spatial_escape_entrapment.percentage, color: "#F59E0B" },
    { label: "Fear of Fear (Panic Interoception)", pct: result.subscales.fear_of_fear_interoception.percentage, color: "#FBBF24" },
    { label: "Safety Tether & Territory Shrinkage", pct: result.subscales.safety_tether_constriction.percentage, color: "#D97706" },
  ];
  ...
  ctx.fillStyle = "#FCD34D";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. DAVID BARLOW CLINICAL PRINCIPLE:", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Agoraphobia is not a fear of physical geography, but an interoceptive terror of having panic sensations with no immediate escape."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen your agoraphobia & comfort zone limits at:", 80, 1200);

  ctx.fillStyle = "#F59E0B";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/agoraphobia", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-agoraphobia-${result.level}.png`, { type: "image/png" });
...
export interface CovertNarcissismScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    defensive_hypersensitivity: { score: number; percentage: number };
    covert_entitlement_grievance: { score: number; percentage: number };
    martyrdom_victim_blame: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Covert Narcissism Screener (Hypersensitive Narcissism Scale - HSNS).
 */
export async function generateCovertNarcissismCard(
  result: CovertNarcissismScoreResult,
  lang: CovertNarcissismCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Indigo Slate
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#080B14");
  bgGrad.addColorStop(0.5, "#101626");
  bgGrad.addColorStop(1, "#06080E");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, width, height);

  // Subtle Indigo/Amethyst Radial Glow
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.25, 40, width * 0.8, height * 0.25, 550);
  radial.addColorStop(0, "rgba(99, 102, 241, 0.16)");
  radial.addColorStop(1, "rgba(99, 102, 241, 0)");
  ctx.fillStyle = radial;
  ctx.fillRect(0, 0, width, height);
  ...
  ctx.fillStyle = "#A5B4FC";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · COVERT NARCISSISM", 108, 115);

  // Category Tag
  ctx.fillStyle = "#818CF8";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("HYPERSENSITIVE NARCISSISM SCALE (HSNS)", 80, 185);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Vulnerable Narcissism Load", 350, curY + 48);

  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "16px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`Clinical HSNS Score: ${result.score} / 36 pts`, 350, curY + 78);

  // Subscales
  curY += 145;
  const subscales = [
    { label: "Defensive Hypersensitivity", pct: result.subscales.defensive_hypersensitivity.percentage, color: "#818CF8" },
    { label: "Covert Entitlement & Grievance", pct: result.subscales.covert_entitlement_grievance.percentage, color: "#A5B4FC" },
    { label: "Martyrdom & Weaponized Victimhood", pct: result.subscales.martyrdom_victim_blame.percentage, color: "#6366F1" },
  ];
  ...
  ctx.fillStyle = "#818CF8";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. JONATHAN CHEEK (HSNS MODEL):", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Vulnerable narcissism operates from shame and grievance: an underlying conviction that one is uniquely gifted, yet tragically misunderstood by a mediocre world."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen your covert narcissism & grievance traits at:", 80, 1200);

  ctx.fillStyle = "#818CF8";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/covert-narcissism", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-covert-narcissism-${result.level}.png`, { type: "image/png" });
...
export interface SomniphobiaScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    bedtime_anticipatory_panic: { score: number; percentage: number };
    loss_of_control_dread: { score: number; percentage: number };
    insomnia_performance_anxiety: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Somniphobia & Sleep Dread Screener (Card #102, Morin & Espie DBAS model).
 */
export async function generateSomniphobiaCard(
  result: SomniphobiaScoreResult,
  lang: SomniphobiaCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Midnight Indigo & Dark Obsidian
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  ...
  // Subtle Indigo / Violet Moon Glow
  ...
  ctx.fillStyle = "#818CF8";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · CARD #102 · MORIN & ESPIE MODEL", 108, 115);

  // Category Tag
  ctx.fillStyle = "#C7D2FE";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("SOMNIPHOBIA & SLEEP DREAD (DBAS COGNITIVE SCALE)", 80, 185);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Sleep Anticipatory Dread Index", 350, curY + 48);

  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "16px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`DBAS Score: ${result.score} / 36 pts`, 350, curY + 78);

  // Subscales
  curY += 145;
  const subscales = [
    { label: "Bedtime Anticipatory Panic", pct: result.subscales.bedtime_anticipatory_panic.percentage, color: "#818CF8" },
    { label: "Loss of Control & Surrender Dread", pct: result.subscales.loss_of_control_dread.percentage, color: "#A855F7" },
    { label: "Insomnia Performance Anxiety", pct: result.subscales.insomnia_performance_anxiety.percentage, color: "#6366F1" },
  ];
  ...
  ctx.fillStyle = "#A5B4FC";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. CHARLES MORIN & DR. COLIN ESPIE PRINCIPLE:", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Sleep effort is an impossible paradox. The more consciously one attempts to force unconsciousness, the more cortical hyperarousal is triggered. True rest begins with psychological surrender."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen your sleep anticipatory anxiety & nighttime dread at:", 80, 1200);

  ctx.fillStyle = "#A5B4FC";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/somniphobia", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-somniphobia-${result.level}.png`, { type: "image/png" });
...
export interface FamilyScapegoatScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    systemic_blame_projection: { score: number; percentage: number };
    smear_campaign_gaslighting: { score: number; percentage: number };
    internalized_defective_identity: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Family Scapegoat Syndrome Screener (Card #103, Mandeville FSA & Bowen model).
 */
export async function generateFamilyScapegoatCard(
  result: FamilyScapegoatScoreResult,
  lang: FamilyScapegoatCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Obsidian Charcoal & Fiery Ember
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  ...
  // Subtle Warm Amber / Coral Radial Glow
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.22, 40, width * 0.8, height * 0.22, 550);
  ...
  ctx.fillStyle = "#FB923C";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · CARD #103 · FSA MANDEVILLE MODEL", 108, 115);

  // Category Tag
  ctx.fillStyle = "#FED7AA";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("FAMILY SCAPEGOAT ABUSE & SYSTEMIC BLAME", 80, 185);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Family Scapegoat Syndrome Index", 350, curY + 48);

  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "16px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`FSA Score: ${result.score} / 36 pts`, 350, curY + 78);

  // Subscales
  curY += 145;
  const subscales = [
    { label: "Systemic Blame Projection", pct: result.subscales.systemic_blame_projection.percentage, color: "#F97316" },
    { label: "Smear Campaign & Gaslighting", pct: result.subscales.smear_campaign_gaslighting.percentage, color: "#EF4444" },
    { label: "Internalized Defective Identity", pct: result.subscales.internalized_defective_identity.percentage, color: "#EAB308" },
  ];
  ...
  ctx.fillStyle = "#FB923C";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("REBECCA MANDEVILLE (FSA MODEL PRINCIPLE):", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"The family scapegoat is rarely the sickest person in the room—they are the truth-teller whose empathy and honesty threaten the dysfunctional family\'s collective denial."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen family scapegoat dynamics & systemic blame at:", 80, 1200);

  ctx.fillStyle = "#FB923C";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/family-scapegoat", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-family-scapegoat-${result.level}.png`, { type: "image/png" });
...
export interface NightEatingScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    circadian_delay_morning_anorexia: { score: number; percentage: number };
    nocturnal_awakenings_ingestion_urge: { score: number; percentage: number };
    evening_dysphoria_sleep_fragmentation: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Night Eating Syndrome (NES) Screener (Card #108, Stunkard & Allison NEQ model).
 */
export async function generateNightEatingCard(
  result: NightEatingScoreResult,
  lang: NightEatingCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Midnight Obsidian & Moon Slate
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#080C14");
  bgGrad.addColorStop(0.5, "#0F172A");
  bgGrad.addColorStop(1, "#1E1B4B");
  ...
  // Subtle Indigo / Amber Radial Glow
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.22, 40, width * 0.8, height * 0.22, 550);
  radial.addColorStop(0, "rgba(99, 102, 241, 0.25)");
  ...
  ctx.fillStyle = "#818CF8";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · CARD #108 · STUNKARD NEQ MODEL", 108, 115);

  // Category Tag
  ctx.fillStyle = "#FDE68A";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NIGHT EATING SYNDROME & CIRCADIAN METABOLISM", 80, 185);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Night Eating Index", 350, curY + 48);

  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "16px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`NEQ Score: ${result.score} / 36 pts`, 350, curY + 78);

  // Subscales
  curY += 145;
  const subscales = [
    { label: "Circadian Delay & Morning Anorexia", pct: result.subscales.circadian_delay_morning_anorexia.percentage, color: "#F59E0B" },
    { label: "Nocturnal Awakenings & Ingestion Urge", pct: result.subscales.nocturnal_awakenings_ingestion_urge.percentage, color: "#818CF8" },
    { label: "Evening Dysphoria & Sleep Fragmentation", pct: result.subscales.evening_dysphoria_sleep_fragmentation.percentage, color: "#F43F5E" },
  ];
  ...
  ctx.fillStyle = "#A5B4FC";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. ALBERT STUNKARD (CIRCADIAN NES PRINCIPLE):", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Night eating is not a lack of willpower or moral weakness—it is an uncoupling of the master circadian clock and peripheral metabolic rhythms under chronic neuroendocrine strain."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen midnight eating urges and circadian phase delay at:", 80, 1200);

  ctx.fillStyle = "#F59E0B";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/night-eating", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-night-eating-${result.level}.png`, { type: "image/png" });
...
export interface PmddScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    luteal_affective_lability_rage: { score: number; percentage: number };
    interpersonal_friction_rejection_pain: { score: number; percentage: number };
    somatic_cognitive_exhaustion_rapid_remission: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Premenstrual Dysphoric Disorder (PMDD) Screener (Card #109, Steiner & Pearlstein DRSP model).
 */
export async function generatePmddCard(
  result: PmddScoreResult,
  lang: PmddCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Plum Midnight Velvet & Rose Crimson Shadow
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#0F0614");
  bgGrad.addColorStop(0.5, "#200C2A");
  bgGrad.addColorStop(1, "#3B0820");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, width, height);

  // Subtle Orchid / Magenta Radial Glow
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.22, 40, width * 0.8, height * 0.22, 550);
  radial.addColorStop(0, "rgba(217, 70, 239, 0.24)");
  radial.addColorStop(0.6, "rgba(244, 63, 94, 0.12)");
  radial.addColorStop(1, "rgba(217, 70, 239, 0)");
  ctx.fillStyle = radial;
  ctx.fillRect(0, 0, width, height);
  ...
  ctx.fillStyle = "#E879F9";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · CARD #109 · STEINER DRSP MODEL", 108, 115);

  // Category Tag
  ctx.fillStyle = "#FBCFE8";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("PREMENSTRUAL DYSPHORIC DISORDER (PMDD)", 80, 185);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("PMDD Luteal Vulnerability", 350, curY + 48);

  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "16px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`DRSP Score: ${result.score} / 36 pts`, 350, curY + 78);

  // Subscales
  curY += 145;
  const subscales = [
    { label: "Luteal Affective Lability & Rage", pct: result.subscales.luteal_affective_lability_rage.percentage, color: "#F43F5E" },
    { label: "Interpersonal Friction & Rejection Pain", pct: result.subscales.interpersonal_friction_rejection_pain.percentage, color: "#C084FC" },
    { label: "Somatic Exhaustion & Rapid Remission", pct: result.subscales.somatic_cognitive_exhaustion_rapid_remission.percentage, color: "#FBBF24" },
  ];
  ...
  ctx.fillStyle = "#F0ABFC";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. MEIR STEINER (PMDD CLINICAL NEUROBIOLOGY):", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"PMDD is not a character flaw or hormonal deficit—it is an acute cellular neurosensitivity to normal neurosteroid flux. Awareness and cycle tracking turn cyclical shame into compassionate biological sovereignty."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen premenstrual dysphoria and cyclical luteal rage at:", 80, 1200);

  ctx.fillStyle = "#F472B6";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/pmdd", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-pmdd-${result.level}.png`, { type: "image/png" });
...
export interface EmetophobiaScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    visceral_hypervigilance_nausea_panic: { score: number; percentage: number };
    contamination_safety_checking_rituals: { score: number; percentage: number };
    interpersonal_travel_avoidance_control: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Emetophobia (Specific Phobia of Vomiting / SPOV) Screener (Card #110, Veale SPOVI model).
 */
export async function generateEmetophobiaCard(
  result: EmetophobiaScoreResult,
  lang: EmetophobiaCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Marine Obsidian & Muted Oceanic Slate
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#040D14");
  bgGrad.addColorStop(0.5, "#0B202D");
  bgGrad.addColorStop(1, "#102F3F");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, width, height);

  // Subtle Cyan / Teal Radial Glow
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.22, 40, width * 0.8, height * 0.22, 550);
  radial.addColorStop(0, "rgba(56, 189, 248, 0.24)");
  radial.addColorStop(0.6, "rgba(45, 212, 191, 0.1)");
  radial.addColorStop(1, "rgba(56, 189, 248, 0)");
  ctx.fillStyle = radial;
  ctx.fillRect(0, 0, width, height);
  ...
  ctx.fillStyle = "#38BDF8";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · CARD #110 · VEALE SPOVI MODEL", 108, 115);

  // Category Tag
  ctx.fillStyle = "#BAE6FD";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("EMETOPHOBIA (SPECIFIC PHOBIA OF VOMITING)", 80, 185);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Emetophobia Phobia Index", 350, curY + 48);

  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "16px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`SPOVI Score: ${result.score} / 36 pts`, 350, curY + 78);

  // Subscales
  curY += 145;
  const subscales = [
    { label: "Visceral Hypervigilance & Nausea Panic", pct: result.subscales.visceral_hypervigilance_nausea_panic.percentage, color: "#38BDF8" },
    { label: "Contamination & Safety Checking Rituals", pct: result.subscales.contamination_safety_checking_rituals.percentage, color: "#2DD4BF" },
    { label: "Interpersonal & Travel Avoidance Control", pct: result.subscales.interpersonal_travel_avoidance_control.percentage, color: "#F59E0B" },
  ];
  ...
  ctx.fillStyle = "#7DD3FC";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("PROF. DAVID VEALE (SPOVI CLINICAL PRINCIPLE):", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Emetophobia is maintained not by the biological act of vomiting, but by the endless safety rituals designed to escape it. Dropping the rituals teaches the brain that nausea is safe."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen fear of vomiting and gastrointestinal panic at:", 80, 1200);

  ctx.fillStyle = "#38BDF8";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/emetophobia", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-emetophobia-${result.level}.png`, { type: "image/png" });
...
export interface NomophobiaScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    device_separation_panic_disconnect: { score: number; percentage: number };
    compulsive_checking_phantom_vibrations: { score: number; percentage: number };
    interpersonal_disconnection_fomo: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Nomophobia & Smartphone Separation Screener (Card #111, Yildirim NMP-Q model).
 */
export async function generateNomophobiaCard(
  result: NomophobiaScoreResult,
  lang: NomophobiaCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Cyber Obsidian & Electric Indigo Slate
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#080C14");
  bgGrad.addColorStop(0.5, "#0F172A");
  bgGrad.addColorStop(1, "#1E1B4B");
  ...
  // Electric Cyan / Neon Amber Radial Glow
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.22, 40, width * 0.8, height * 0.22, 550);
  radial.addColorStop(0, "rgba(6, 182, 212, 0.25)");
  radial.addColorStop(0.6, "rgba(245, 158, 11, 0.1)");
  radial.addColorStop(1, "rgba(6, 182, 212, 0)");
  ctx.fillStyle = radial;
  ctx.fillRect(0, 0, width, height);
  ...
  ctx.fillStyle = "#22D3EE";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · CARD #111 · YILDIRIM NMP-Q MODEL", 108, 115);

  // Category Tag
  ctx.fillStyle = "#A5F3FC";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NOMOPHOBIA & DIGITAL SEPARATION ANXIETY", 80, 185);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Nomophobia Dependency Index", 350, curY + 48);

  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "16px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`NMP-Q Score: ${result.score} / 36 pts`, 350, curY + 78);

  // Subscales
  curY += 145;
  const subscales = [
    { label: "Device Separation Panic & Disconnect", pct: result.subscales.device_separation_panic_disconnect.percentage, color: "#06B6D4" },
    { label: "Compulsive Checking & Phantom Vibrations", pct: result.subscales.compulsive_checking_phantom_vibrations.percentage, color: "#6366F1" },
    { label: "Interpersonal Disconnection & FOMO", pct: result.subscales.interpersonal_disconnection_fomo.percentage, color: "#F43F5E" },
  ];
  ...
  ctx.fillStyle = "#67E8F9";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. ÇAĞAN YILDIRIM (NMP-Q RESEARCH PRINCIPLE):", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Nomophobia reveals that the smartphone has transitioned from an external communication tool into a biological surrogate for emotional regulation and social safety."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen smartphone separation anxiety and nomophobia at:", 80, 1200);

  ctx.fillStyle = "#06B6D4";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/nomophobia", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-nomophobia-${result.level}.png`, { type: "image/png" });
...
export interface ClaustrophobiaScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    suffocation_fear_air_flow_panic: { score: number; percentage: number };
    physical_restriction_entrapment_dread: { score: number; percentage: number };
    anticipatory_avoidance_escape_monitoring: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Claustrophobia & Enclosed Space Screener (Card #112, Rachman CLQ model).
 */
export async function generateClaustrophobiaCard(
  result: ClaustrophobiaScoreResult,
  lang: ClaustrophobiaCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Atmospheric Iron & Carbon Navy
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#0A0F16");
  bgGrad.addColorStop(0.5, "#101A26");
  bgGrad.addColorStop(1, "#182635");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, width, height);

  // Radial Sky Beam / Atmospheric Glow
  const radial = ctx.createRadialGradient(width * 0.82, height * 0.2, 40, width * 0.82, height * 0.2, 540);
  ...
  ctx.fillStyle = "#38BDF8";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · CARD #112 · RACHMAN CLQ MODEL", 108, 115);

  // Category Tag
  ctx.fillStyle = "#BAE6FD";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("CLAUSTROPHOBIA & ENCLOSED SPACE ANXIETY", 80, 185);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Claustrophobia Distress Index", 350, curY + 48);

  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "16px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`CLQ Score: ${result.score} / 36 pts`, 350, curY + 78);

  // Subscales
  curY += 145;
  const subscales = [
    { label: "Suffocation Fear & Air Flow Panic", pct: result.subscales.suffocation_fear_air_flow_panic.percentage, color: "#38BDF8" },
    { label: "Physical Restriction & Entrapment Dread", pct: result.subscales.physical_restriction_entrapment_dread.percentage, color: "#F59E0B" },
    { label: "Anticipatory Avoidance & Escape Monitoring", pct: result.subscales.anticipatory_avoidance_escape_monitoring.percentage, color: "#FB7185" },
  ];
  ...
  ctx.fillStyle = "#7DD3FC";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("PROF. JACK RACHMAN (CLQ RESEARCH PRINCIPLE):", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Claustrophobia is primarily fueled not by geometric confinement alone, but by the catastrophic misinterpretation of restricted ventilation and motor entrapment."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen claustrophobia and enclosed space anxiety at:", 80, 1200);

  ctx.fillStyle = "#38BDF8";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/claustrophobia", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-claustrophobia-${result.level}.png`, { type: "image/png" });
...
export interface CyberchondriaScoreResult {
  score: number;
  percentage: number;
  level: string;
  profile: {
  ...
  subscales: {
    compulsive_symptom_searching_rabbit_holes: { score: number; percentage: number };
    catastrophic_distress_bodily_hypervigilance: { score: number; percentage: number };
    reassurance_seeking_medical_mistrust: { score: number; percentage: number };
  };
}

/**
 * Generates an aesthetic high-resolution Instagram Story share card (1080x1350)
 * for the Cyberchondria & Online Health Anxiety Screener (Card #113, Starcevic CSS model).
 */
export async function generateCyberchondriaCard(
  result: CyberchondriaScoreResult,
  lang: CyberchondriaCardLang = "en"
): Promise<{ dataUrl: string; blob: Blob; file: File }> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  ...
  // Background: Deep Emerald Obsidian & Tech Cyan Slate
  const bgGrad = ctx.createLinearGradient(0, 0, width, height);
  bgGrad.addColorStop(0, "#040F0C");
  bgGrad.addColorStop(0.5, "#081E19");
  bgGrad.addColorStop(1, "#0A2822");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, width, height);

  // Luminous Diagnostic Mint & Cyber Glow
  const radial = ctx.createRadialGradient(width * 0.8, height * 0.22, 40, width * 0.8, height * 0.22, 550);
  radial.addColorStop(0, "rgba(16, 185, 129, 0.25)");
  radial.addColorStop(0.6, "rgba(6, 182, 212, 0.12)");
  radial.addColorStop(1, "rgba(16, 185, 129, 0)");
  ctx.fillStyle = radial;
  ctx.fillRect(0, 0, width, height);
  ...
  ctx.fillStyle = "#34D399";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("NUJU CLINICAL · CARD #113 · STARCEVIC CSS MODEL", 108, 115);

  // Category Tag
  ctx.fillStyle = "#A7F3D0";
  ctx.font = "600 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("CYBERCHONDRIA & ONLINE HEALTH ANXIETY", 80, 185);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Cyberchondria Severity Index", 350, curY + 48);

  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "16px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText(`CSS Score: ${result.score} / 36 pts`, 350, curY + 78);

  // Subscales
  curY += 145;
  const subscales = [
    { label: "Compulsive Symptom Searching", pct: result.subscales.compulsive_symptom_searching_rabbit_holes.percentage, color: "#10B981" },
    { label: "Catastrophic Distress & Bodily Vigilance", pct: result.subscales.catastrophic_distress_bodily_hypervigilance.percentage, color: "#F43F5E" },
    { label: "Reassurance Seeking & Medical Mistrust", pct: result.subscales.reassurance_seeking_medical_mistrust.percentage, color: "#06B6D4" },
  ];
  ...
  ctx.fillStyle = "#6EE7B7";
  ctx.font = "bold 20px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("DR. VLADAN STARCEVIC (CSS RESEARCH PRINCIPLE):", 145, curY + 40);

  ctx.fillStyle = "rgba(255, 255, 255, 0.85)";
  ctx.font = "italic 19px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  const quote = '"Cyberchondria occurs when the online search for medical certainty paradoxically exposes the user to the most catastrophic, improbable diagnoses, locking the nervous system in alarm."';
  const quoteLines = wrapText(ctx, quote, width - 290);
  let qY = curY + 75;
  quoteLines.forEach((l) => {
    ctx.fillText(l, 145, qY);
  ...
  ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
  ctx.font = "bold 22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("Screen cyberchondria and online health anxiety at:", 80, 1200);

  ctx.fillStyle = "#10B981";
  ctx.font = "900 30px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
  ctx.fillText("nuju.app/quiz/cyberchondria", 80, 1245);

  const dataUrl = canvas.toDataURL("image/png", 0.95);
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Failed blob"))), "image/png", 0.95);
  });
  const file = new File([blob], `nuju-cyberchondria-${result.level}.png`, { type: "image/png" });
