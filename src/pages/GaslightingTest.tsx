...
import {
  ...
  calculateGaslightingScore,
  GaslightingLang,
} from "@/data/gaslighting";
import { GaslightingShareCardModal } from "@/components/GaslightingShareCardModal";
import { toast } from "sonner";

interface GaslightingTestProps {
  defaultLang?: GaslightingLang;
}

const GaslightingTest: React.FC<GaslightingTestProps> = ({ defaultLang = "en" }) => {
  const [lang, setLang] = useState<GaslightingLang>(defaultLang);
  ...
  const currentQ = GASLIGHTING_QUESTIONS[currentIndex];
  const progressPercent = Math.round(
    ((currentIndex + 1) / GASLIGHTING_QUESTIONS.length) * 100
  );
  ...
  const handleSelectOption = (score: number) => {
    ...
    if (currentIndex < GASLIGHTING_QUESTIONS.length - 1) {
  ...
  const result = isCompleted ? calculateGaslightingScore(answers) : null;

  const copyResults = () => {
    if (!result) return;
    const text = `My Psychological Gaslighting & Manipulation Screener Result: ${
      result.profile.title[lang] || result.profile.title.en
    } (${result.percentage}% exposure index - ${
      result.profile.badge[lang] || result.profile.badge.en
    }). Verify emotional reality free: https://nuju.app/quiz/gaslighting`;
    navigator.clipboard.writeText(text);
    toast.success("Result summary copied to clipboard!");
  };

  return (
    <div className="min-h-screen bg-[#0D0714] text-neutral-100 selection:bg-rose-500/30">
      <SEOHead
        title="Gaslighting & Emotional Manipulation Test: Free 12-Item Diagnostic"
        description="Are you being gaslit or second-guessing your reality? Take our free 12-item psychological manipulation test to verify emotional reality, recognize manipulation tactics, and regain confidence."
        canonical="https://nuju.app/quiz/gaslighting"
        language={lang}
      />

      {/* Top Banner Navigation */}
      <header className="sticky top-0 z-40 border-b border-neutral-800/80 bg-[#0D0714]/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-1 text-xs">
            {(["en", "id", "de", "fr", "es"] as GaslightingLang[]).map((l) => (
              <button
                key={l}
                onClick={() => setLang(l)}
      ...
      <div className="mx-auto max-w-4xl px-4 pt-4 sm:px-6">
        <AdSenseBanner slot="gaslighting-top" className="my-2" />
      </div>
            ...
            <div className="rounded-3xl border border-rose-500/20 bg-gradient-to-br from-rose-500/10 via-neutral-900/40 to-neutral-950 p-6 sm:p-8 shadow-xl">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-rose-500/10 border border-rose-500/30 px-3 py-1 text-xs font-bold text-rose-400">
                  ...
                  STERN PSYCHOLOGICAL MANIPULATION SCALE
                </span>
                <span className="text-xs font-semibold text-neutral-400">
                  {currentIndex + 1} / {GASLIGHTING_QUESTIONS.length}
                </span>
              </div>
              ...
              <p className="text-sm text-neutral-400 leading-relaxed">
                Determine if covert psychological manipulation, reality distortion, or chronic invalidation are eroding your confidence and sense of truth.
              </p>
            ...
            >
              ...
              <div className="rounded-3xl border border-rose-500/30 bg-gradient-to-br from-rose-500/15 via-neutral-900 to-neutral-950 p-6 sm:p-10 shadow-2xl relative overflow-hidden">
                <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                  ...
                  <div className="text-right">
                    <span className="text-xs text-neutral-400 block font-medium">Manipulation Exposure</span>
                    <span className="text-2xl sm:text-3xl font-black text-rose-400">{result.percentage}%</span>
                  </div>
                </div>
                ...
                <p className="text-base sm:text-lg font-medium text-rose-200/90 leading-relaxed mb-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-6 border-t border-neutral-800">
                  <div className="p-4 rounded-2xl bg-neutral-950/60 border border-neutral-800">
                    <span className="text-xs font-bold text-rose-400 block mb-1 uppercase">Reality Distortion</span>
                    <span className="text-2xl font-black text-white">{result.subscales.reality_distortion.percentage}%</span>
                    <p className="text-[11px] text-neutral-400 mt-1">Memory & Perception Doubt</p>
                  </div>
                  <div className="p-4 rounded-2xl bg-neutral-950/60 border border-neutral-800">
                    <span className="text-xs font-bold text-amber-400 block mb-1 uppercase">Hyper-Vigilance</span>
                    <span className="text-2xl font-black text-white">{result.subscales.hyper_vigilance.percentage}%</span>
                    <p className="text-[11px] text-neutral-400 mt-1">Walking on Eggshells</p>
                  </div>
                  <div className="p-4 rounded-2xl bg-neutral-950/60 border border-neutral-800">
                    <span className="text-xs font-bold text-violet-400 block mb-1 uppercase">Apology Reflex</span>
                    <span className="text-2xl font-black text-white">{result.subscales.apology_reflex.percentage}%</span>
                    <p className="text-[11px] text-neutral-400 mt-1">Compulsive Self-Blame</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                <button
                  ...
                  className="flex-1 min-w-[200px] flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-rose-500 to-violet-600 hover:opacity-95 px-6 py-3.5 text-sm font-bold text-white transition shadow-lg shadow-rose-500/20"
                >
              ...
              <div className="rounded-3xl border border-neutral-800 bg-neutral-900/60 p-6 sm:p-8">
                <div className="flex items-center gap-2 text-rose-400 font-bold text-sm mb-3">
                  ...
                  <span>PSYCHOLOGICAL ANALYSIS & EROSION CYCLE</span>
                </div>
              ...
              <div className="rounded-3xl border border-neutral-800 bg-neutral-900/60 p-6 sm:p-8">
                <div className="flex items-center gap-2 text-rose-400 font-bold text-sm mb-4">
                  ...
                  <span>GROUNDING & BOUNDARY PROTOCOLS</span>
                </div>
                <div className="space-y-3">
              ...
              <div className="rounded-3xl border border-rose-500/20 bg-gradient-to-r from-rose-500/10 to-transparent p-6 sm:p-8 text-center">
                <span className="text-xs font-bold text-rose-400/80 uppercase tracking-widest block mb-2">
                  Reality Anchor & Affirmation
                </span>
              ...
              <div className="rounded-3xl border border-neutral-800 bg-gradient-to-br from-neutral-900 via-neutral-950 to-neutral-900 p-6 sm:p-8 text-center space-y-4">
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  Protect Your Truth with an Anonymous, Encrypted Journal
                </h3>
                <p className="text-xs sm:text-sm text-neutral-400 max-w-md mx-auto leading-relaxed">
                  Gaslighting thrives in isolation and unrecorded memories. Timestamp conversations, ground your perceptions, and track emotional patterns privately in Nuju.
                </p>
              ...
              <div className="my-6">
                <AdSenseBanner slot="gaslighting-bottom" className="my-2" />
              </div>
            </motion.div>
          )
        )}
...
export default GaslightingTest;
