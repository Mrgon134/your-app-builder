...
import {
  HYPER_INDEPENDENCE_QUESTIONS,
  calculateHyperIndependenceScore,
  HyperIndependenceLang,
} from "@/data/hyper-independence";
import { HyperIndependenceShareCardModal } from "@/components/HyperIndependenceShareCardModal";
import { toast } from "sonner";

interface HyperIndependenceTestProps {
  defaultLang?: HyperIndependenceLang;
}

const HyperIndependenceTest: React.FC<HyperIndependenceTestProps> = ({
  defaultLang = "en",
}) => {
  const [lang, setLang] = useState<HyperIndependenceLang>(defaultLang);
  ...
  const currentQ = HYPER_INDEPENDENCE_QUESTIONS[currentIndex];
  const progressPercent = Math.round(
    ((currentIndex + 1) / HYPER_INDEPENDENCE_QUESTIONS.length) * 100
  );
  ...
  const handleSelectOption = (score: number) => {
    ...
    if (currentIndex < HYPER_INDEPENDENCE_QUESTIONS.length - 1) {
  ...
  const result = isCompleted ? calculateHyperIndependenceScore(answers) : null;

  const copyResults = () => {
    if (!result) return;
    const text = `My Hyper-Independence Profile: ${
      result.profile.title[lang] || result.profile.title.en
    } (${result.percentage}% trauma armor index - ${
      result.profile.badge[lang] || result.profile.badge.en
    }). Screen your trauma-based self-reliance free: https://nuju.app/quiz/hyper-independence`;
    navigator.clipboard.writeText(text);
    toast.success("Result summary copied to clipboard!");
  };

  return (
    <div className="min-h-screen bg-[#070D14] text-neutral-100 selection:bg-sky-500/30">
      <SEOHead
        title="Hyper-Independence Test: Free Trauma-Fawn & Armor Diagnostic"
        description="Do you refuse to ask for help even when overwhelmed or exhausted? Take our free 12-item clinical screener to see if your self-reliance is a trauma defense mechanism."
        canonical="https://nuju.app/quiz/hyper-independence"
        language={lang}
      />

      {/* Top Banner Navigation */}
      <header className="sticky top-0 z-40 border-b border-neutral-800/80 bg-[#070D14]/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-1 text-xs">
            {(["en", "id", "de", "fr", "es"] as HyperIndependenceLang[]).map((l) => (
              <button
                key={l}
                onClick={() => setLang(l)}
      ...
      <div className="mx-auto max-w-4xl px-4 pt-4 sm:px-6">
        <AdSenseBanner slot="hyper-independence-top" className="my-2" />
      </div>
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
        {!isCompleted ? (
          ...
          <div className="space-y-6">
            {/* Header / Intro Card */}
            <div className="rounded-3xl border border-sky-500/20 bg-gradient-to-br from-sky-500/10 via-neutral-900/40 to-neutral-950 p-6 sm:p-8 shadow-xl">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-sky-500/10 border border-sky-500/30 px-3 py-1 text-xs font-bold text-sky-300">
                  <Shield className="h-3.5 w-3.5" />
                  ATTACHMENT & TRAUMA DEFENSE MODEL
                </span>
                <span className="text-xs font-semibold text-neutral-400">
                  {currentIndex + 1} / {HYPER_INDEPENDENCE_QUESTIONS.length}
                </span>
              </div>
              ...
              <p className="text-sm text-neutral-400 leading-relaxed">
                Identify if your refusal to accept help and emotional solitude are strengths or an armor born from early betrayal, parentification, or relational trauma.
              </p>

              {/* Progress Bar */}
              <div className="mt-6 h-2 w-full overflow-hidden rounded-full bg-neutral-800">
                <motion.div
                  className="h-full bg-gradient-to-r from-sky-500 via-teal-500 to-emerald-400"
                  initial={{ width: 0 }}
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[11px] font-semibold text-sky-400/80 uppercase">
                      ...
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-2xl border transition flex items-center justify-between group ${
                          isSelected
                            ...
                            : "border-neutral-800 bg-neutral-950/40 hover:border-sky-500/50 hover:bg-neutral-800/40 text-neutral-300"
                        }`}
                      >
                        <span className="text-sm sm:text-base font-medium pr-4 leading-relaxed">
                          {option.label[lang] || option.label.en}
                        </span>
                        <ChevronRight className="h-4 w-4 text-neutral-500 group-hover:text-sky-400 shrink-0 transition" />
                      </button>
                    );
                  })}
                </div>
          ...
          result && (
            ...
            >
              ...
              <div className="rounded-3xl border border-sky-500/30 bg-gradient-to-br from-sky-500/15 via-neutral-900 to-neutral-950 p-6 sm:p-10 shadow-2xl relative overflow-hidden">
                <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-sky-500/20 border border-sky-500/40 px-3.5 py-1 text-xs font-bold text-sky-300 shadow-sm">
                ...
                <p className="text-base sm:text-lg italic text-sky-200/90 font-serif mb-6">
                ...
                {/* Subscales Tri-Grid */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-sky-500/20">
                  <div className="rounded-2xl bg-neutral-900/80 border border-neutral-800 p-4">
                    ...
                    <div className="text-2xl font-black text-white mb-1">
                      {result.subscales.solitary_armor.percentage}%
                    </div>
                  ...
                  <div className="rounded-2xl bg-neutral-900/80 border border-neutral-800 p-4">
                    ...
                    <div className="text-2xl font-black text-white mb-1">
                      {result.subscales.vulnerability_phobia.percentage}%
                    </div>
                    <span className="text-xs text-neutral-400">
                      Emotional Lockdown & Containment
                    </span>
                  </div>
                  ...
                  <div className="rounded-2xl bg-neutral-900/80 border border-neutral-800 p-4">
                    ...
                    <div className="text-2xl font-black text-white mb-1">
                      {result.subscales.threat_detection.percentage}%
                    </div>
                    <span className="text-xs text-neutral-400">
                      Anticipatory Disappointment & Mistrust
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-sky-500 to-teal-600 hover:opacity-90 active:scale-[0.98] text-white font-bold text-sm shadow-lg shadow-sky-500/20 transition"
                  >
              ...
              <div className="rounded-3xl border border-neutral-800 bg-neutral-900/60 p-6 sm:p-8 space-y-4">
                <div className="flex items-center gap-2 text-sky-400">
                  ...
                  <h2 className="text-lg sm:text-xl font-bold text-white">
                    Neuro-Defense Analysis & Interdependence Capacity
                  </h2>
                </div>
              ...
              <div className="rounded-3xl border border-neutral-800 bg-neutral-900/60 p-6 sm:p-8 space-y-4">
                <div className="flex items-center gap-2 text-teal-400">
                  ...
                  <h2 className="text-lg sm:text-xl font-bold text-white">
                    Progressive Micro-Trust & Co-Regulation Roadmap
                  </h2>
                </div>
                <div className="space-y-3">
              ...
              <AdSenseBanner slot="hyper-independence-mid" className="my-4" />
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-neutral-800">
                ...
                >
                  <span>View All 42+ Diagnostics</span>
                </Link>
              </div>
            </motion.div>
          )
      ...
      {/* Share Card Modal */}
      {result && (
        <HyperIndependenceShareCardModal
...
export default HyperIndependenceTest;
