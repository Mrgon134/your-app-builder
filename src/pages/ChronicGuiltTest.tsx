...
import {
  CHRONIC_GUILT_QUESTIONS,
  calculateChronicGuiltScore,
  ChronicGuiltLang,
} from "@/data/chronic-guilt";
import { ChronicGuiltShareCardModal } from "@/components/ChronicGuiltShareCardModal";
import { toast } from "sonner";

interface ChronicGuiltTestProps {
  defaultLang?: ChronicGuiltLang;
}

const ChronicGuiltTest: React.FC<ChronicGuiltTestProps> = ({
  defaultLang = "en",
}) => {
  const [lang, setLang] = useState<ChronicGuiltLang>(defaultLang);
  ...
  const currentQ = CHRONIC_GUILT_QUESTIONS[currentIndex];
  const progressPercent = Math.round(
    ((currentIndex + 1) / CHRONIC_GUILT_QUESTIONS.length) * 100
  );
  ...
  const handleSelectOption = (score: number) => {
    ...
    if (currentIndex < CHRONIC_GUILT_QUESTIONS.length - 1) {
  ...
  const result = isCompleted ? calculateChronicGuiltScore(answers) : null;

  const copyResults = () => {
    if (!result) return;
    const text = `My Chronic Guilt & Self-Sabotage Profile: ${
      result.profile.title[lang] || result.profile.title.en
    } (${result.percentage}% toxic guilt load - ${
      result.profile.badge[lang] || result.profile.badge.en
    }). Assess your subconscious guilt and penance patterns free: https://nuju.app/quiz/chronic-guilt`;
    navigator.clipboard.writeText(text);
    toast.success("Result summary copied to clipboard!");
  };

  return (
    <div className="min-h-screen bg-[#0C0609] text-neutral-100 selection:bg-rose-500/30">
      <SEOHead
        title="Chronic Guilt & Self-Sabotage Test: Free Clinical Subconscious Remorse Screener"
        description="Do you constantly feel guilty for existing, relaxing, or other people's bad moods? Take our free 12-item clinical screener for toxic remorse, omnipotent responsibility, and subconscious self-punishment."
        canonical="https://nuju.app/quiz/chronic-guilt"
        language={lang}
      />

      {/* Top Banner Navigation */}
      <header className="sticky top-0 z-40 border-b border-neutral-800/80 bg-[#0C0609]/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-1 text-xs">
            {(["en", "id", "de", "fr", "es"] as ChronicGuiltLang[]).map((l) => (
              <button
                key={l}
                onClick={() => setLang(l)}
      ...
      <div className="mx-auto max-w-4xl px-4 pt-4 sm:px-6">
        <AdSenseBanner slot="chronic-guilt-top" className="my-2" />
      </div>
            ...
            <div className="rounded-3xl border border-rose-500/20 bg-gradient-to-br from-rose-500/10 via-neutral-900/40 to-neutral-950 p-6 sm:p-8 shadow-xl">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-rose-500/10 border border-rose-500/30 px-3 py-1 text-xs font-bold text-rose-300">
                  ...
                  KUGEL-JONES GUILT INVENTORY (GIC)
                </span>
                <span className="text-xs font-semibold text-neutral-400">
                  {currentIndex + 1} / {CHRONIC_GUILT_QUESTIONS.length}
                </span>
              </div>
              ...
              <p className="text-sm text-neutral-400 leading-relaxed">
                Differentiate healthy remorse from toxic neurotic guilt. Uncover whether omnipotent responsibility and subconscious self-punishment are sabotaging your joy and success.
              </p>
            ...
            >
              ...
              <div className="rounded-3xl border border-rose-500/30 bg-gradient-to-br from-rose-500/15 via-neutral-900 to-neutral-950 p-6 sm:p-10 shadow-2xl relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-rose-500/20">
                  <div className="rounded-2xl bg-neutral-900/80 border border-neutral-800 p-4">
                    ...
                    <div className="text-2xl font-black text-white mb-1">
                      {result.subscales.omnipotent_responsibility.percentage}%
                    </div>
                    <span className="text-xs text-neutral-400">
                      Emotional Rescuing & Blame Burden
                    </span>
                  </div>
                  ...
                  <div className="rounded-2xl bg-neutral-900/80 border border-neutral-800 p-4">
                    ...
                    <div className="text-2xl font-black text-white mb-1">
                      {result.subscales.punitive_self_sabotage.percentage}%
                    </div>
                    <span className="text-xs text-neutral-400">
                      Subconscious Penance & Deprivation
                    </span>
                  </div>

                  <div className="rounded-2xl bg-neutral-900/80 border border-neutral-800 p-4">
                    <span className="text-[11px] font-bold text-pink-400 uppercase tracking-wider block mb-1">
                      Unworthiness Anxiety
                    </span>
                    <div className="text-2xl font-black text-white mb-1">
                      {result.subscales.unworthiness_anxiety.percentage}%
                    </div>
                    <span className="text-xs text-neutral-400">
                      Existential Apology & Invalidation
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-rose-500 to-amber-600 hover:opacity-90 active:scale-[0.98] text-white font-bold text-sm shadow-lg shadow-rose-500/20 transition"
                  >
              ...
              <div className="rounded-3xl border border-neutral-800 bg-neutral-900/60 p-6 sm:p-8 space-y-4">
                <div className="flex items-center gap-2 text-rose-400">
                  ...
                  <h2 className="text-lg sm:text-xl font-bold text-white">
                    Psychological Analysis & Super-Ego Dynamics
                  </h2>
                </div>
              ...
              <div className="rounded-3xl border border-neutral-800 bg-neutral-900/60 p-6 sm:p-8 space-y-4">
                <div className="flex items-center gap-2 text-amber-400">
                  ...
                  <h2 className="text-lg sm:text-xl font-bold text-white">
                    Unconditional Repair & Boundaried Compassion Protocol
                  </h2>
                </div>
                <div className="space-y-3">
              ...
              <AdSenseBanner slot="chronic-guilt-mid" className="my-4" />
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-neutral-800">
                ...
                >
                  <span>Explore People Pleasing Test</span>
...
export default ChronicGuiltTest;
