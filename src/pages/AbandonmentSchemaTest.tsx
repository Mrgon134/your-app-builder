...
import {
  ABANDONMENT_SCHEMA_QUESTIONS,
  ABANDONMENT_SCHEMA_RESULTS,
  ABANDONMENT_SCHEMA_OPTIONS,
  ABANDONMENT_SCHEMA_SUBSCALE_INFO,
  getAbandonmentSchemaResult,
  calculateAbandonmentSchemaSubscales,
  AbandonmentSchemaCardLang,
} from "@/data/abandonment-schema";
import { AbandonmentSchemaShareCardModal } from "@/components/AbandonmentSchemaShareCardModal";
import { AbandonmentSchemaScoreResult } from "@/lib/generate-quiz-card";

export default function AbandonmentSchemaTest() {
  const [lang, setLang] = useState<AbandonmentSchemaCardLang>("en");
  const [answers, setAnswers] = useState<Record<number, number>>({});
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [isCompleted, setIsCompleted] = useState(false);
  const [shareModalOpen, setShareModalOpen] = useState(false);

  const questions = ABANDONMENT_SCHEMA_QUESTIONS;
  const currentQuestion = questions[currentQuestionIndex];
  const progressPercent = Math.round(((currentQuestionIndex + 1) / questions.length) * 100);

  const answerLabels = useMemo(() => {
    return ABANDONMENT_SCHEMA_OPTIONS.map((opt) => ({
      val: opt.value,
  ...
  const scoreResult: AbandonmentSchemaScoreResult = useMemo(() => {
    let total = 0;
    questions.forEach((q) => {
    ...
    const levelObj = getAbandonmentSchemaResult(total);
    const rawSubscales = calculateAbandonmentSchemaSubscales(answers);
    ...
    return {
      ...
      subscales: {
        anticipatory_desertion_terror: {
          score: rawSubscales.anticipatory_desertion_terror,
          percentage: Math.round((rawSubscales.anticipatory_desertion_terror / 12) * 100),
        },
        clinging_testing_protest: {
          score: rawSubscales.clinging_testing_protest,
          percentage: Math.round((rawSubscales.clinging_testing_protest / 12) * 100),
        },
        relational_fragility_conviction: {
          score: rawSubscales.relational_fragility_conviction,
          percentage: Math.round((rawSubscales.relational_fragility_conviction / 12) * 100),
        },
      },
    };
  }, [answers, questions]);

  const activeProfile = getAbandonmentSchemaResult(scoreResult.score);

  // Schema.org Structured Data
  const jsonLdData = {
    "@context": "https://schema.org",
    "@type": ["MedicalWebPage", "Quiz"],
    name: "Abandonment Schema & Relational Panic Screener (Dr. Jeffrey Young Schema Therapy Model)",
    headline: "Abandonment Schema Test: Measure Relational Panic, Clinging & Fear of Being Left",
    description:
      "Assess whether delayed replies, partner independence, or relational distance trigger primal survival panic according to Dr. Jeffrey Young's Schema Therapy framework.",
    url: "https://nuju.app/quiz/abandonment-schema",
    medicalAudience: "Patient",
    about: {
      "@type": "MedicalCondition",
      name: "Early Maladaptive Abandonment Schema & Attachment Panic",
      possibleTreatment: [
        { "@type": "MedicalTherapy", name: "Schema Therapy Limited Reparenting" },
        { "@type": "MedicalTherapy", name: "Somatic Vagus Nerve Grounding for Attachment Alarms" },
        { "@type": "MedicalTherapy", name: "Private Voice Diary Self-Soothing" },
      ],
    },
  ...
  return (
    <>
      <SEOHead
        title="Abandonment Schema Test: Screen Fear of Being Left & Relational Panic"
        description="Take the evidence-based 12-item Abandonment Schema Screener (Dr. Jeffrey Young Schema Therapy model). Free, anonymous, and instant insight into attachment panic and protest behaviors."
        canonical="https://nuju.app/quiz/abandonment-schema"
      />
      ...
      <div className="min-h-screen bg-neutral-950 text-neutral-100 selection:bg-rose-500/30">
        ...
        <header className="border-b border-neutral-800 bg-neutral-900/60 backdrop-blur-md sticky top-0 z-40">
          <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5 bg-neutral-800/80 p-1 rounded-full border border-neutral-700/60 text-xs">
              {(["en", "id", "de", "fr", "es"] as AbandonmentSchemaCardLang[]).map((l) => (
                <button
                  key={l}
                  onClick={() => setLang(l)}
            ...
            <div>
              ...
              <div className="mb-8 p-6 rounded-3xl border border-neutral-800 bg-gradient-to-br from-neutral-900/80 via-neutral-900/40 to-neutral-950 shadow-xl">
                <div className="flex items-center gap-2 text-rose-400 text-xs font-bold uppercase tracking-wider mb-2">
                  ...
                  <span>Dr. Jeffrey Young Schema Therapy Model</span>
                </div>
                <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight mb-2">
                  Abandonment Schema &amp; Relational Panic Screener
                </h1>
                <p className="text-sm text-neutral-400 leading-relaxed">
                  Does a delayed text ignite panic in your chest? Do you test loved ones or break up first to escape being dumped?
                  This 12-item screener benchmarks anticipatory desertion terror, protest clinging, and relational fragility convictions.
                </p>
                ...
                <div className="w-full h-2 bg-neutral-800 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-rose-500 to-sky-400 transition-all duration-300 rounded-full"
              ...
              <div className="p-8 rounded-3xl border border-neutral-800 bg-neutral-900/70 shadow-xl backdrop-blur-sm">
                <div className="text-xs font-semibold text-rose-400 uppercase tracking-wider mb-3">
                  Dimension: {ABANDONMENT_SCHEMA_SUBSCALE_INFO[currentQuestion.subscale].name[lang] || ABANDONMENT_SCHEMA_SUBSCALE_INFO[currentQuestion.subscale].name.en}
                </div>
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="p-8 rounded-3xl border border-rose-500/30 bg-gradient-to-br from-rose-950/40 via-neutral-900 to-neutral-950 shadow-2xl relative overflow-hidden">
                ...
                <div className="flex items-baseline gap-2 mb-6">
                  ...
                  <span className="text-xs font-bold text-rose-300/80 ml-2">
                    ({scoreResult.percentage}% Abandonment Load)
                  </span>
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div className="p-4 rounded-2xl bg-neutral-900/80 border border-neutral-800">
                      ...
                      <div className="text-xl font-bold text-white mb-2">
                        {scoreResult.subscales.anticipatory_desertion_terror.score}/12 pts
                      </div>
                      <div className="w-full h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-rose-500 rounded-full"
                          style={{ width: `${scoreResult.subscales.anticipatory_desertion_terror.percentage}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="p-4 rounded-2xl bg-neutral-900/80 border border-neutral-800">
                      ...
                      <div className="text-xl font-bold text-white mb-2">
                        {scoreResult.subscales.clinging_testing_protest.score}/12 pts
                      </div>
                      <div className="w-full h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-pink-500 rounded-full"
                          style={{ width: `${scoreResult.subscales.clinging_testing_protest.percentage}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="p-4 rounded-2xl bg-neutral-900/80 border border-neutral-800">
                      ...
                      <div className="text-xl font-bold text-white mb-2">
                        {scoreResult.subscales.relational_fragility_conviction.score}/12 pts
                      </div>
                      <div className="w-full h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-sky-400 rounded-full"
                          style={{ width: `${scoreResult.subscales.relational_fragility_conviction.percentage}%` }}
                        />
                      </div>
                    </div>
                  </div>
              ...
              <div className="p-6 sm:p-8 rounded-3xl border border-neutral-800 bg-neutral-900/70 shadow-lg">
                <div className="flex items-center gap-2 text-rose-400 text-xs font-bold uppercase tracking-wider mb-2">
                  ...
                  <span>Dr. Jeffrey Young's Schema Architecture</span>
                </div>
                <h3 className="text-lg font-bold text-white mb-3">
                  Why Physical Distance Triggers an Ancient Childhood Survival Alarm
                </h3>
                <p className="text-sm text-neutral-300 leading-relaxed mb-4">
                  {activeProfile.psychology[lang] || activeProfile.psychology.en}
                </p>
                <div className="p-4 rounded-2xl bg-rose-950/30 border border-rose-800/30 text-xs text-rose-200 leading-relaxed">
                  <strong>The Schema Paradox:</strong> The tragic reality of the abandonment schema is that the desperate, frantic attempts to retain a partner (testing loyalty, interrogating, lashing out, or pre-emptively breaking up) are precisely what suffocate and drive secure partners away. Healing requires cultivating your own internal anchor.
                </div>
              </div>
              ...
              <div className="p-6 sm:p-8 rounded-3xl border border-neutral-800 bg-neutral-900/70 shadow-lg">
                <div className="flex items-center gap-2 text-sky-400 text-xs font-bold uppercase tracking-wider mb-2">
                  ...
                  <span>Sovereign Anchoring Protocol</span>
                </div>
                <h3 className="text-lg font-bold text-white mb-4">
                  3 Schema Therapy Practices to Soothe the Vulnerable Child
                </h3>
              ...
              <div className="p-8 rounded-3xl border border-rose-500/30 bg-gradient-to-br from-rose-900/30 via-neutral-900 to-neutral-950 shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="space-y-2 text-center sm:text-left">
                  <span className="px-3 py-1 rounded-full bg-rose-500/20 text-rose-300 text-xs font-bold uppercase tracking-wider">
                    Zero-Knowledge Encrypted Lifeline
                  </span>
                  <h3 className="text-xl sm:text-2xl font-black text-white">
                    Soothe Your Abandonment Alarm in Nuju
                  </h3>
                  <p className="text-xs sm:text-sm text-neutral-300 max-w-xl">
                    When you feel the terrifying wave of loneliness and urge to send desperate texts, speak directly into
                    Nuju's zero-knowledge encrypted audio sanctuary. Give voice to the terrified child within you and discover that you will never abandon yourself.
                  </p>
                </div>
                <a
                  href="https://nuju.app"
        ...
        {/* Share Card Modal */}
        <AbandonmentSchemaShareCardModal
