...
import {
  PERCEIVED_QUESTIONS,
  calculatePerceivedScore,
  PerceivedLang,
} from "@/data/fear-of-being-perceived";
import { FearOfBeingPerceivedShareCardModal } from "@/components/FearOfBeingPerceivedShareCardModal";
import { toast } from "sonner";

interface FearOfBeingPerceivedTestProps {
  defaultLang?: PerceivedLang;
}

const FearOfBeingPerceivedTest: React.FC<FearOfBeingPerceivedTestProps> = ({
  defaultLang = "en",
}) => {
  const [lang, setLang] = useState<PerceivedLang>(defaultLang);
  ...
  const currentQ = PERCEIVED_QUESTIONS[currentIndex];
  const progressPercent = Math.round(
    ((currentIndex + 1) / PERCEIVED_QUESTIONS.length) * 100
  );
  ...
  const handleSelectOption = (score: number) => {
    ...
    if (currentIndex < PERCEIVED_QUESTIONS.length - 1) {
  ...
  const result = isCompleted ? calculatePerceivedScore(answers) : null;

  const copyResults = () => {
    if (!result) return;
    const text = `My Fear of Being Perceived Profile: ${
      result.profile.title[lang] || result.profile.title.en
    } (${result.percentage}% spotlight anxiety load - ${
      result.profile.badge[lang] || result.profile.badge.en
    }). Screen your visibility fear and social concealment free: https://nuju.app/quiz/fear-of-being-perceived`;
    navigator.clipboard.writeText(text);
    toast.success("Result summary copied to clipboard!");
  };

  return (
    <div className="min-h-screen bg-[#030A07] text-neutral-100 selection:bg-emerald-500/30">
      <SEOHead
        title="Fear of Being Perceived Test: Free Spotlight & Concealment Diagnostic"
        description="Do you dread being looked at, noticed, or speaking in public? Take our free 12-item clinical screener based on Gilovich's Spotlight Effect and Social Evaluative Threat models."
        canonical="https://nuju.app/quiz/fear-of-being-perceived"
        language={lang}
      />

      {/* Top Banner Navigation */}
      <header className="sticky top-0 z-40 border-b border-neutral-800/80 bg-[#030A07]/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-1 text-xs">
            {(["en", "id", "de", "fr", "es"] as PerceivedLang[]).map((l) => (
              <button
                key={l}
                onClick={() => setLang(l)}
      ...
      <div className="mx-auto max-w-4xl px-4 pt-4 sm:px-6">
        <AdSenseBanner slot="fear-perceived-top" className="my-2" />
      </div>
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
        {!isCompleted ? (
          ...
          <div className="space-y-6">
            {/* Header / Intro Card */}
            <div className="rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 via-neutral-900/40 to-neutral-950 p-6 sm:p-8 shadow-xl">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 px-3 py-1 text-xs font-bold text-emerald-300">
                  ...
                  GILOVICH SPOTLIGHT & SOCIAL THREAT MODEL
                </span>
                <span className="text-xs font-semibold text-neutral-400">
                  {currentIndex + 1} / {PERCEIVED_QUESTIONS.length}
                </span>
              </div>
              ...
              <p className="text-sm text-neutral-400 leading-relaxed">
                Discover why existing in public, being looked at, or having a visible body triggers intense mortification, social masking, and post-exposure shame spirals.
              </p>

              {/* Progress Bar */}
              <div className="mt-6 h-2 w-full overflow-hidden rounded-full bg-neutral-800">
                <motion.div
                  className="h-full bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-400"
                  initial={{ width: 0 }}
                      ...
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-2xl border transition flex items-center justify-between group ${
                          isSelected
                            ...
                            : "border-neutral-800 bg-neutral-950/40 hover:border-emerald-500/50 hover:bg-neutral-800/40 text-neutral-300"
                        }`}
                      >
                        <span className="text-sm sm:text-base font-medium pr-4 leading-relaxed">
                          {option.label[lang] || option.label.en}
                        </span>
                        <ChevronRight className="h-4 w-4 text-neutral-500 group-hover:text-emerald-400 shrink-0 transition" />
                      </button>
                    );
                  })}
                </div>
          ...
          result && (
            ...
            >
              ...
              <div className="rounded-3xl border border-emerald-500/30 bg-gradient-to-br from-emerald-500/15 via-neutral-900 to-neutral-950 p-6 sm:p-10 shadow-2xl relative overflow-hidden">
                <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/20 border border-emerald-500/40 px-3.5 py-1 text-xs font-bold text-emerald-300 shadow-sm">
                ...
                <p className="text-base sm:text-lg italic text-emerald-200/90 font-serif mb-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-emerald-500/20">
                  <div className="rounded-2xl bg-neutral-900/80 border border-neutral-800 p-4">
                    ...
                    <div className="text-2xl font-black text-white mb-1">
                      {result.subscales.visibility_dread.percentage}%
                    </div>
                    <span className="text-xs text-neutral-400">
                      Panic When Noticed or Looked At
                    </span>
                  </div>
                  ...
                  <div className="rounded-2xl bg-neutral-900/80 border border-neutral-800 p-4">
                    ...
                    <div className="text-2xl font-black text-white mb-1">
                      {result.subscales.covert_concealment.percentage}%
                    </div>
                    <span className="text-xs text-neutral-400">
                      Stealth Camouflage & Shrinking Presence
                    </span>
                  </div>
                  ...
                  <div className="rounded-2xl bg-neutral-900/80 border border-neutral-800 p-4">
                    ...
                    <div className="text-2xl font-black text-white mb-1">
                      {result.subscales.post_exposure_rumination.percentage}%
                    </div>
                    <span className="text-xs text-neutral-400">
                      Vulnerability Hangover & Mental Replay
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:opacity-90 active:scale-[0.98] text-white font-bold text-sm shadow-lg shadow-emerald-500/20 transition"
                  >
              ...
              <div className="rounded-3xl border border-neutral-800 bg-neutral-900/60 p-6 sm:p-8 space-y-4">
                <div className="flex items-center gap-2 text-emerald-400">
                  ...
                  <h2 className="text-lg sm:text-xl font-bold text-white">
                    Spotlight Effect & Social Evaluative Threat (SET)
                  </h2>
                </div>
              ...
              <div className="rounded-3xl border border-neutral-800 bg-neutral-900/60 p-6 sm:p-8 space-y-4">
                <div className="flex items-center gap-2 text-teal-400">
                  ...
                  <h2 className="text-lg sm:text-xl font-bold text-white">
                    Progressive Micro-Visibility & Somatic Safety Protocol
                  </h2>
                </div>
                <div className="space-y-3">
              ...
              <AdSenseBanner slot="fear-perceived-mid" className="my-4" />
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-neutral-800">
                ...
                >
                  <span>Explore Social Battery Test</span>
      ...
      {/* Share Card Modal */}
      {result && (
        <FearOfBeingPerceivedShareCardModal
...
export default FearOfBeingPerceivedTest;
